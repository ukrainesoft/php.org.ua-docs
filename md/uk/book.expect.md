---
navigation:
  - evwatcher.stop.html: '« EvWatcher::stop'
  - intro.expect.html: Введение »
  - index.html: PHP Manual
  - refs.fileprocess.process.html: Модули для управления процессами программ
title: Expect
---
# Expect

-   [Введение](intro.expect.html)
-   [Встановлення та налаштування](expect.setup.html)
    -   [Вимоги](expect.requirements.html)
    -   [Установка](expect.installation.html)
    -   [Налаштування під час виконання](expect.configuration.html)
    -   [Типи ресурсів](expect.resources.html)
-   [Обумовлені константи](expect.constants.html)
-   [Приклади](expect.examples.html)
    -   [Приклади використання Expect](expect.examples-usage.html)
-   [Функции Expect](ref.expect.html)
    -   [expectexpectl](function.expect-expectl.html) — Очікує, поки виведення потоку не співпаде з одним із шаблонів, або доки не закінчиться максимальний час очікування, або повернеться EOF
    -   [expectpopen](function.expect-popen.html) — Запускає команду через командну оболонку Bourne та відкриває для процесу потік PTY
