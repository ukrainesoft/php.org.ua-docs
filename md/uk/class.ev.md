...
Прапори, що передаються в [Ev::run()](ev.run.html), або [EvLoop::run()](evloop.run.html)
...
Прапори, що передаються в [Ev::stop()](ev.stop.html) , або [EvLoop::stop()](evloop.stop.html)
...
Повертає найглибший запит [Ev::run()](ev.run.html) (або [EvLoop::run()](evloop.run.html)

**`Ev::BREAK_ALL`**

Завершує всі вкладені дзвінки [Ev::run()](ev.run.html) (або [EvLoop::run()](evloop.run.html)
...
Дескриптор файлу у спостерігачі [EvIo](class.evio.html) доступний для читання.

**`Ev::WRITE`**

Дескриптор файлу у спостерігачі [EvIo](class.evio.html) доступний для запису.

**`Ev::TIMER`**

[EvTimer](class.evtimer.html) спостерігає за перевищенням ліміту часу.

**`Ev::PERIODIC`**

[EvPeriodic](class.evperiodic.html) спостерігає за перевищенням ліміту часу.
...
Шлях, вказаний у спостерігачі [EvStat](class.evstat.html) змінив свої атрибути.

**`Ev::IDLE`**

Спостерігач [EvIdle](class.evidle.html) працює, коли решта спостерігачів нічого не робить.

**`Ev::PREPARE`**

Усі спостерігачі [EvPrepare](class.evprepare.html) викликані рівно перед стартом [Ev::run()](ev.run.html) таким чином, спостерігачі [EvPrepare](class.evprepare.html) є останніми викликаними спостерігачами перед тим, як цикл подій засне чи опитає нові події.

**`Ev::CHECK`**

Усі спостерігачі [EvCheck](class.evcheck.html) поміщені в чергу відразу після того, як [Ev::run()](ev.run.html) зібрав нові події, але до того, як вони викличуть якусь callback-функцію для отриманих подій. Таким чином, спостерігачі [EvCheck](class.evcheck.html) будуть викликані раніше, ніж будь-які інші спостерігачі з таким самим або нижчим пріоритетом у цій ітерації циклу подій.

**`Ev::EMBED`**

Вбудований цикл подій, заданий у спостерігачі [EvEmbed](class.evembed.html) вимагає себе уваги.

**`Ev::CUSTOM`**

Ніколи не надсилається (або іншим чином використовується) бібліотекою `libev` самостійно, але може вільно використовуватись користувачами `libev` для сигналізуючих спостерігачів (тобто за допомогою [EvWatcher::feed()](evwatcher.feed.html)
...
`kqueue` - бекенде, що використовується в більшості систем BSD. Спостерігач [EvEmbed](class.evembed.html) може бути використаний для вставки одного циклу (з бекендом kqueue) в інший. Наприклад, можна спробувати створити цикл подій з бекендом `kqueue` та використовувати його тільки для сокетів.
...
Пробувати всі бекенди (крім зіпсованих). Не рекомендується використовувати безпосередньо. Тут потрібно використовувати побітові операції (тобто . **`Ev::BACKEND_ALL`** & ~ **`Ev::BACKEND_KQUEUE`** ). Використати [Ev::recommendedBackends()](ev.recommendedbackends.html) , або не ставити жодного бекенда взагалі.
...
> **Зауваження**
> 
> Є методи, що дозволяють отримати доступ до *циклу подій за умовчанням* у класі **Єв** (наприклад, [Ev::iteration()](ev.iteration.html) [Ev::depth()](ev.depth.html) і т.д.). Для *користувальницьких циклів* (створених за допомогою **EvLoop:: construct()**) ці значення можуть бути доступні через відповідні властивості та методи класу [EvLoop](class.evloop.html)
> 
> Примірник циклу подій за умовчанням може бути вилучений за допомогою методу [EvLoop::defaultLoop()](evloop.defaultloop.html)
