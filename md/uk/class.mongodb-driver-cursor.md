...
Клас **MongoDBDriverCursor** містить результати команди MongoDB command або запиту та може бути повернутий [MongoDB\\Driver\\Manager::executeCommand()](mongodb-driver-manager.executecommand.html) або [MongoDB\\Driver\\Manager::executeQuery()](mongodb-driver-manager.executequery.html) відповідно.
...
Як [MongoDB\\Driver\\Manager::executeCommand()](mongodb-driver-manager.executecommand.html), так [MongoDB\\Driver\\Manager::executeQuery()](mongodb-driver-manager.executequery.html), повертають свої результати у вигляді об'єкту **MongoDBDriverCursor**
...
Коли скрипт продюсера (producer) все ще запущений, може бути виконаний другий скрипт для читання вставлених документів за допомогою хвостового (tailable) курсора, позначеного параметрами `tailable` і `awaitData` для [MongoDB\\Driver\\Query::\_\_construct()](mongodb-driver-query.construct.html)
