...
| **`CLSCTX_SERVER`**(int) | Вказує код сервера, всередині процесний, локальний чи віддалений. Задається побітовим АБО констант **`CLSCTX_INPROC_SERVER`** **`CLSCTX_LOCAL_SERVER`**и**`CLSCTX_REMOTE_SERVER`** |  |
| **`CLSCTX_ALL`**(int) | Позначає весь контекст класу. Задається побітовим АБО констант **`CLSCTX_INPROC_HANDLER`**и**`CLSCTX_SERVER`** |  |
...
| **`VT_DATE`**(int) | Значення з плаваючою точкою розміром 64 біта, що становить кількість днів (не секунд) з `31 грудня 1899`Например,`1 січня 1900` одно `2.0` `2 січня 1900` одно `3.0`, і т. д. Зберігається так само, як і **`VT_R8`** |  |
