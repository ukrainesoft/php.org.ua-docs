...
За промовчанням використовується значення директиви [default\_socket\_timeout](filesystem.configuration.html#ini.default-socket-timeout) із файлу php.ini.
...
> Додаткові опції контексту можуть підтримуватись [нижележащим транспортным протоколом](transports.inet.html). Для потоків `http://`, це стосується опцій контексту для транспортного протоколу `tcp://`. Для потоків `https://`, це стосується опцій контексту для транспортного протоколу `ssl://`

> **Зауваження** **Рядок статусу HTTP**  
> Коли ця обгортка потоку слідує за переадресацією, `wrapper_data`, що повертається функцією [stream\_get\_meta\_data()](function.stream-get-meta-data.html), необов'язково містить рядок статусу HTTP, який насправді відноситься до змісту даних за індексом `0`
> 
> ```
> array (
>   'wrapper_data' =>
