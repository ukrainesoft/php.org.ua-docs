...
Як тільки ви зібрали PHP з підтримкою cURL, вже можна використовувати функції cURL. Робота з cURL завжди починається з виклику [curlinit()](function.curl-init.md), потім встановлюються необхідні параметри за допомогою [curlsetopt()](function.curl-setopt.md), і виконується потрібна операція викликом [curlexec()](function.curl-exec.md), після чого викликом [curlclose()](function.curl-close.md) сеанс роботи завершується. Нижче наведений приклад використовує функції cURL для збереження стартової сторінки сайту example.com у файл:
