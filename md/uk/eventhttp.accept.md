...
`<?php$base = new EventBase();$http = new EventHttp($base);$addresses = array (     8091 => "127.0.0.1", =8                       | 0;$socket = array();foreach ($addresses as $port => $ip) {   echo $ip, " ", $port, PHP_EOL; $socket[$i] = socket_create(AF_INET, SOCK_STREAM, SOL_TCP); if (!socket_bind($socket[$i], $ip, $port)) {        exit("помилка socket_bind
");   }                                                                 
...
";});$signal = Event::signal($base, SIGINT, function () use ($base) {    echo "Пійман SIGINT. Зупиняємо...
