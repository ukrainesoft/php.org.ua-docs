...
PHP надає [вбудоване API хешування паролів](book.password.html), яке безпечно працює і з [хешированием](function.password-hash.html) і з [перевіркою паролів](function.password-verify.html)
...
Врахуйте, що якщо ви використовуєте функцію [crypt()](function.crypt.html) Для перевірки пароля, то вам потрібно застерегти себе від атак за часом, застосовуючи порівняння рядків, що займає постійний час. Ні оператори PHP [\= и ===](language.operators.comparison.html), ні функція [strcmp()](function.strcmp.html) не є такими. Функція ж [passwordverify()](function.password-verify.html) робить те, що потрібно. Настійно рекомендується використовувати [вбудоване API хешування паролів](book.password.html)якщо є така можливість.
...
Наступна діаграма показує формат значення, що повертається функціями [crypt()](function.crypt.html) або [passwordhash()](function.password-hash.html). Як бачимо, вони містять повну інформацію про алгоритм і солі, необхідні майбутньої перевірки пароля.
