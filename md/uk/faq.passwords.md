...
1.  [Почему я должен хешировать пароли пользователей в моем приложении?](#faq.passwords.hashing)
2.  [Почему популярные хеширующие функции, такие как md5 и sha1 не подходят для паролей?](#faq.passwords.fasthash)
3.  [Если популярные хеширующие функции не подходят, как же я тогда должен хешировать свои пароли?](#faq.passwords.bestpractice)
4.  [Что такое соль?](#faq.passwords.salt)
5.  [Как я должен хранить свою соль?](#faq.password.storing-salts)
...
**Чому популярні функції хешування, такі як [md5()](function.md5.html) і [sha1()](function.sha1.html) не підходять для паролів?**
...
PHP надає [встроенное API хеширования паролей](book.password.html), яке безпечно працює і з [хешированием](function.password-hash.html) і з [проверкой паролей](function.password-verify.html)

Іншою можливістю є функція [crypt()](function.crypt.html)що підтримує кілька алгоритмів хешування. При використанні цієї функції ви можете бути впевненим, що вибраний вами алгоритм доступний, оскільки PHP містить власну реалізацію кожного алгоритму, що підтримується, навіть у випадку, якщо якісь з них не підтримуються вашою системою.
...
Врахуйте, що якщо ви використовуєте функцію [crypt()](function.crypt.html) Для перевірки пароля, то вам потрібно застерегти себе від атак за часом, застосовуючи порівняння рядків, що займає постійний час. Ні оператори PHP [\== и ===](language.operators.comparison.html), ні функція [strcmp()](function.strcmp.html) не є такими. Функція ж [password\_verify()](function.password-verify.html) робить те, що потрібно. Настійно рекомендується використовувати [встроенное API хеширования паролей](book.password.html)якщо є така можливість.
...
[password\_hash()](function.password-hash.html) створює випадкову сіль у разі, якщо вона не була передана, і найчастіше це найкращий та безпечний вибір.

**Як я маю зберігати свою сіль?**

При використанні функції [password\_hash()](function.password-hash.html) або [crypt()](function.crypt.html), що повертається вже містить сіль як частина створеного хеша. Це значення потрібно зберігати як є у вашій базі даних, так як воно містить також інформацію про хешируючу функцію, яка використовувалася, і може бути безпосередньо передано до функцій [password\_verify()](function.password-verify.html) або [crypt()](function.crypt.html) під час перевірки пароля.

Наступна діаграма показує формат значення, що повертається функціями [crypt()](function.crypt.html) або [password\_hash()](function.password-hash.html). Як бачимо, вони містять повну інформацію про алгоритм і солі, необхідні майбутньої перевірки пароля.
