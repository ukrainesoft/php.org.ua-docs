...
Насамперед, основною причиною реалізації механізму складання сміття є зменшення розміру пам'яті, що використовується за допомогою чищення циклічних посилань, яка відбувається при досягненні відповідних умов. У реалізації PHP це відбувається, як тільки заповниться кореневий буфер або при виклику функції [gc\_collect\_cycles()](function.gc-collect-cycles.html). На графіку нижче наведено використання пам'яті скрипта, запущеного в PHP 5.2 та PHP 5.3, без урахування пам'яті, що використовується самим PHP під час запуску.
...
Ми запустимо скрипт двічі: із включеною опцією [zend.enable\_gc](info.configuration.html#ini.zend.enable-gc) і без неї.
