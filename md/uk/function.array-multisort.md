...
-   \*\*`SORT_FLAG_CASE`**\- Можна об'єднувати (бінарним АБО) з**`SORT_STRING`**или**`SORT_NATURAL`\*\*для сортування без урахування регістру
...
Обидві константи **`SORT_STRING`**и**`SORT_REGULAR`** враховують регістр при сортуванні, рядки, що починаються з великої літери, будуть виставлені раніше рядків, що починаються з малої літери.
