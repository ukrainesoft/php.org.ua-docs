...
> **Зауваження** :
> 
> До PHP 7.3.0,**bcmul()** могла повернути результат з меншою кількістю цифр після десяткової точки, ніж задає параметр `scale`. Це могло статися тільки у випадку, якщо результату не потрібна `scale`К Прикладу:
> 
