...
> **Зауваження** :
> 
> До PHP 7.3.0,**bcmul()** могла повернути результат з меншою кількістю цифр після десяткової точки, ніж задає параметр `scale`. Це могло статися тільки у випадку, якщо результату не потрібна `scale`К примеру:
> 
> **Пример #2 Пример**bcmul()\*\* із заданою точністю\*\*
> 
> ```php
> <?php
> echo bcmul('5', '2', 2); // надрукує "10", а не "10.00"
> ?>
> ```
