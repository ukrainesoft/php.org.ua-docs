...
Обратите внимание, что значение параметра`permissions` не переводиться автоматично у вісімкову систему числення, тому, щоб переконатися в тому, що режим був встановлений правильно, передуйте нулю (0) значення, що передається в параметрі `permissions`. . Рядки, такі як "g+w", не працюватимуть належним чином.
