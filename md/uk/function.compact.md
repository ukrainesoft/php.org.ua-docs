...
Для кожного з переданих параметрів, функція **compact()** шукає змінну із зазначеним ім'ям у поточній таблиці символів і додає їх у масив, що виводиться так, що ім'я змінної стає ключем, а вміст змінної стає значенням цього ключа. Коротше кажучи, вона протилежна функції [extract()](function.extract.html)
...
> 
> Так як [переменные переменных](language.variables.variable.html) не можуть бути використані з [суперглобальными массивами](language.variables.superglobals.html) всередині функцій, суперглобальні масиви не можуть бути передані в **compact()**
