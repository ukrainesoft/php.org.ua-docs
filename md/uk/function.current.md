...
> **Зауваження**: Результати виклику **current()** на порожньому масиві та на масиві, внутрішній покажчик якого вказує на кінець елементів, не відрізняються від елемента масиву типу bool зі значенням **`false`**. Для коректного обходу масиву, який може містити \*\*`false`\*\*дивіться керуючу конструкцію [foreach](control-structures.foreach.md). Якщо ви хочете використовувати функцію **current()** та при цьому коректно відстежувати кінець масиву, використовуйте функцію [key()](function.key.md) на тому ж елементі, на якому використовували **current()** і перевіряйте її результат на сувору нерівність **`null`**
