...
` <?php$sql = "SELECT id, name, breed, weight FROM animals ORDER BY breed";$stmt = db2_prepare($conn, $sql);$result = db2_execute($st y ($stmt)) {    printf ("%-5d %-16s %-32s %10s
...
` <?php$sql = "SELECT id, name, breed, weight FROM animals ORDER BY breed";$result = db2_exec($stmt, $sql, array('cursor' => DB2_SCR while($row = db2_fetch_array($result, $i)) {    printf ("%-5d %-16s %-32s %10s
