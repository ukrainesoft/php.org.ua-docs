...
Для отримання додаткової інформації про постійні з'єднання дивіться [Постоянные соединения с базами данных](features.persistent-connections.html)

Виклик [db2\_close()](function.db2-close.html) у постійному з'єднанні завжди повертає **`true`**, але базове клієнтське з'єднання DB2 залишається відкритим і чекає на обслуговування наступного відповідного запиту **db2pconnect()**
...
Передача значення DB2TRUSTEDCONTEXTENABLE включає довірений контекст цього дескриптора з'єднання. Параметр не можна встановити за допомогою [db2\_set\_option()](function.db2-set-option.html)
