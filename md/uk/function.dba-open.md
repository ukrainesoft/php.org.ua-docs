...
Ім'я [обработчика](dba.requirements.html), який буде використовуватися для доступу на шляху `path`. Йому передаються всі необов'язкові параметри, задані в **dbaopen()**
...
Драйвери `db1` `db2` `db3` `db4` `dbm` `gdbm`, і `ndbm` підтримують єдиний додатковий параметр `$filemode`що має те саме значення, що і параметр `$mode` функції [chmod()](function.chmod.html) і за умовчанням дорівнює `0644`
