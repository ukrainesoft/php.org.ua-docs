...
` <?php// відкриємо в режимі читання і записи$db = dbase_open('/tmp/test.dbf', 2);if ($db) {  // отримаємо старий запис  $row ; // скинемо помітку 'deleted' unset($row['deleted']); // Встановимо в поле дати поточну дату $row['date'] = date('Ymd'); // Перетворимо $row в масив  $row = array_values($row); // Замінимо запис  dbase_replace_record($db, $row, 1); dbase_close($db);}?> `
