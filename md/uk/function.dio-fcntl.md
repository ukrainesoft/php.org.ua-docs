...
-   **`F_SETFL`**\- встановлює файловий дескриптор прапори, задані в`args`, такі як**`O_APPEND`** **`O_NONBLOCK`**или**`O_ASYNC`**. Для використання\*\*`O_ASYNC`\*\*вам необхідно скористатися модулем[PCNTL](ref.pcntl.md)
    

`args`

`args` - це асоціативний масив, якщо `cmd`установлен в\*\*`F_SETLK`**или**`F_SETLLW`\*\*, з такими ключами:

-   `start`\- зміщення початку блокування
    
-   `length`\- Розмір заблокованої зони. 0 означає кінець файлу
    
-   `whence`\- Залежить від l\_start: можливо\*\*`SEEK_SET`\*\* **`SEEK_END`**и**`SEEK_CUR`**
