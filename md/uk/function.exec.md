...
**exec**(string `$command`, array `&$output` = **`null`**, int
`&$result_code` = **`null`**): string\|false
...
` <?php// виводить ім'я користувача, від імені якого запущено процесс php/httpd// (застосовується до системам з командою "whoami" в системному шляху)$output=n'$=' output, $retval);echo "Поверне статус $retval і значення:
";print_r($output);?> `
