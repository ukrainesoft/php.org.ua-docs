...
Припиняє виконання скрипту. [Функції відключення](function.register-shutdown-function.md) та [деструктори об'єкта](language.oop5.decon.md#language.oop5.decon.destructor) будуть
...
` <?php$filename = '/path/to/data-file';$file = fopen($filename, 'r')   oror exit("Неможливо відкрити файл ($filename)");?> `
...
` <?phpclass Foo{    public function __destruct()    {       echo 'Деініціалізувати: ' . __METHOD__ . '()' . PHP_EOL; }}function shutdown(){   echo 'Завершити: ' . __FUNCTION__ . '()' . PHP_EOL;}$foo = new Foo();register_shutdown_function('shutdown');exit();echo 'Ця рядок не буде виведена.';?> `
