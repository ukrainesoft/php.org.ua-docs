...
` <?php$fp = fopen("/tmp/lock.txt", "r+");if (flock($fp, LOCK_EX)) { // виконуємо ексклюзивне блокування    ftruncate($fp, 0 // очищаємо файл    fwrite($fp, "Що-небудь пишем|сюди
...
` <?php$fp = fopen('/tmp/lock.txt', 'r+');/* Включаємо параметр LOCK_NB в операції LOCK_EX */if(!flock($fp, LOCK_EX  LOCK_N вдалося отримати блокування'; exit(-1);}/* ... */fclose($fp);?> `
