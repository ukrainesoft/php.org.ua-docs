...
` <?phpif (!($fp =fopen('date.txt', 'w'))) {   return;}fprintf($fp, "%04d-%02d-%02d", $year, $month, $day);// запише форматовану ISO-дату в файл date.txt?> `

**Приклад #2 **fprintf()**: форматування грошових величин**

` <?phpif (!($fp = fopen('currency.txt', 'w'))) {   return;}$money1 = 68.75;$money2 = 54.35;$money = $money $money виведе "123.1";$len = fprintf($fp, '%01.2f',$$money); / використовуйте повертається із fprintf значення для визначення кількості записаних байт?> `
