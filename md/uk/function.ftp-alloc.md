...
`$size`, string `&$response` = **`null`**): bool
...
` <?php$file = "/home/user/myfile";/* з'єднання з сервером */$ftp = ftp_connect('ftp.example.com');$login_result = ftp_login($ftp, 'anonymous', ' user@example.com');if(ftp_alloc($ftp,filesize($file),$result)) { echo "Місце на сервері успішно зарезервовано. Відправляю $file.
