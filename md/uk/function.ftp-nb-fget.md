...
` <?php// відкрити файл для запису$file = 'index.php';$fp = fopen($file, 'w');$ftp = ftp_connect($ftp_server);$login_result = ftp_login($ft ftp_user_name, $ftp_user_pass); ; // продовження скачування ...   $ret = ftp_nb_continue($ftp);}if ($ret != FTP_FINISHED) {   echo "При|завантаженні файлу відбулася..." exit(1);}// закриття файлаfclose($fp);?> `
