...
` <?php$file = 'index.php';$fp = fopen($file, 'r');$ftp = ftp_connect($ftp_server);$login_result = ftp_login($ftp, $ftp_user_name, $ftp_user // Початок завантаження$ret = ftp_nb_fput($ftp, $file, $fp, FTP_BINARY);while ($ret == FTP_MOREDATA) {   //  виробляємо                          // продовження завантаження ...   $ret = ftp_nb_continue($ftp);}if ($ret != FTP_FINISHED) {   echo "При завантаженні файла відбулася" exit(1);}fclose($fp);?> `
