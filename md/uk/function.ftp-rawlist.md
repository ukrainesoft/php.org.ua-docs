...
string `$directory`, bool `$recursive` = **`false`**): array\|false
...
`<?php//установка з'єднання$ftp = ftp_connect($ftp_server);// перевірка імені користувача і пароля$login_result = ftp_login($ftp, $ftp_user_name, $ftp ($ftp, '/');// закриття з'єднанняftp_close($ftp);// виведення буфераvar_dump($buff);?> `
