...
` <?php$dir = 'www/';// установка з'єднання$ftp = ftp_connect($ftp_server);// перевірка імені користувача і пароля$login_result u003d ftp_login($ftp, $ft видалення директорії $dirif (ftp_rmdir($ftp, $dir)) {    echo "Директорія $dir віддалена
