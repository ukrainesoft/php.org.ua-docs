...
` <?php$file = 'somefile.txt';// установка з'єднання$ftp = ftp_connect($ftp_server);// перевірка імені пользователя і пароля$login_result = ftp_login($ft$ отримання|розміру| file";}// закриття з'єднанняftp_close($ftp);?> `
