...
| `''` |  | Порожній рядок активує режим визначення кодування із файлу скрипта (Zend multibyte), [default\_charset](ini.core.html#ini.default-charset) та поточної локалі (дивіться [nl\_langinfo()](function.nl-langinfo.html) і [setlocale()](function.setlocale.html)) у зазначеному порядку. Не рекомендується використовувати. |
