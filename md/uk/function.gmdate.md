...
Ця функція ідентична функції [date()](function.date.html) за винятком того, що повертає час за Гринвічем (GMT).

### Список параметрів

`format`

Формат дати типу рядок (string). Дивіться параметри форматування для функції [date()](function.date.html)
