...
Ця функція ідентична функції [mktime()](function.mktime.html), за винятком того, що аргументи є часом за Грінвічем (GMT) . **gmmktime()** всередині використовує [mktime()](function.mktime.html), тому можуть бути використані лише ті дати місцевого часу, які коректно надаються в цьому часовому поясі.

Подібно до функції [mktime()](function.mktime.html), аргументи можуть бути опущені в порядку праворуч наліво, в цьому випадку вони передбачаються рівними відповідним компонентам поточного часу за Гринвічем.

Виклик **gmmktime()** без аргументів застарів. Для отримання поточної позначки часу можна використовувати [time()](function.time.html)
