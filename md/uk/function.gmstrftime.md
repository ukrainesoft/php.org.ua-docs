Форматує дату/час за Грінвічем з урахуванням поточної локалі

-   [« gmmktime](function.gmmktime.html)
    
-   [idate »](function.idate.html)
    
-   [PHP Manual](index.html)
    
-   [Функции даты и времени](ref.datetime.html)
    
-   Форматує дату/час за Грінвічем з урахуванням поточної локалі
    

# gmstrftime

(PHP 4, PHP 5, PHP 7, PHP 8)

gmstrftime — Форматує дату/час за Грінвічем з урахуванням поточної локалі

**Увага**

Функція оголошена *застарілої* у PHP 8.1.0. Покладатися на цю функцію не рекомендується.

Є такі альтернативи:

-   [gmdate()](function.gmdate.html)
-   [IntlDateFormatter::format()](intldateformatter.format.html)

### Опис

```methodsynopsis
gmstrftime(string $format, ?int $timestamp = null): string|false
```

Ця функція ідентична функції [strftime()](function.strftime.html) за винятком того, що повертає час за Гринвічем (GMT). Наприклад, при запуску на системі, де встановлено Eastern Standard Time (GMT-0500), перший рядок з прикладу нижче виведе "Dec 31 1998 20:00:00", тоді як другий - "Jan 01 1999 01:00:00".

### Список параметрів

`format`

Дивіться опис функції [strftime()](function.strftime.html)

`timestamp`

Необов'язковий параметр `timestamp` являє собою мітку часу типу int, за умовчанням рівну поточному локальному часу, якщо `timestamp` не вказано або **`null`**. Іншими словами, значення за замовчуванням дорівнює результату функції [time()](function.time.html)

### Значення, що повертаються

Повертає рядок, відформатований згідно з вказаним форматом та з використанням тимчасової мітки з параметра `timestamp` або поточного локального часу, якщо тимчасова мітка не була вказана. Назви місяців, днів тижня та інших мовнозалежних рядків будуть показані з урахуванням налаштувань поточної локалі, встановлених за допомогою функції [setlocale()](function.setlocale.html). У разі виникнення помилки повертає **`false`**

### список змін

| Версия | Описание                                  |
|--------|-------------------------------------------|
|        | `timestamp` тепер допускає значення null. |

### Приклади

**Приклад #1 Приклад використання функції **gmstrftime()****

```php
<?php
setlocale(LC_TIME, 'en_US');
echo strftime("%b %d %Y %H:%M:%S", mktime(20, 0, 0, 12, 31, 98)) . "\n";
echo gmstrftime("%b %d %Y %H:%M:%S", mktime(20, 0, 0, 12, 31, 98)) . "\n";
?>
```

### Дивіться також

-   [strftime()](function.strftime.html) - Форматує поточну дату/час з урахуванням поточних налаштувань локалі