...
` <?php// завантажити зображення$im = imagecreatefromgif('phplogo.gif');// отримати найближчі кольори на зображенні$colors = array();$colors[] = imagecolorresolve($5,5 5 ;$colors[] = imagecolorresolve($im, 0, 0, 200);// висновокprint_r($colors);imagedestroy($im);?> `
