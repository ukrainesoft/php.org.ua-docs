...
`<?php// Створення зображення розміром 300x100$im = imagecreate(300, 100); , 0, 0);// Установка синього кольори фонуimagecolorset($im, $bg, 0, 0, 255);// Виведення зображення в браузерheader('Content-Type: image/$');png imagedestroy($im);?> `
