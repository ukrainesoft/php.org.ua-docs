...
Іншими словами, **imagecopyresampled()** бере прямокутну ділянку з `src_image` із шириною `src_width` та заввишки `src_height`на координатах`src_x`,`src_y` і поміщає його у прямокутну ділянку зображення `dst_image`шириной`dst_width` та заввишки `dst_height`на координатах`dst_x`,`dst_y`
