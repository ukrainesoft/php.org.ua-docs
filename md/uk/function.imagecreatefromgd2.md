...
` <?php// завантаження gd2 зображення$im = imagecreatefromgd2('./test.gd2');// застосування ефекту к зображенню, цей код// інвертує колір if(function_exists| IMG_FILTER_NEGATE);}// збереження зображенняimagegd2($im, './test_updated.gd2');imagedestroy($im);?> `
