...
` <?php// Для цього прикладу нам спочатку потрібно дізнатися розмір зображення$image u003d getimagesize('./test.gd2');// Створення зображення$im u003d imagecreatefromgd   ($image[0] / 2) - 6, ($image[1] / 2) - 6);// Додамо рельєф на зображенняif(function_exists('imagefilter')){    imagefilter($im, / збереження зображенняimagegd2($im, './test_emboss.gd2');imagedestroy($im);?> `
