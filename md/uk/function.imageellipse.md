...
`<?php// Створення порожнього зображення. $image, 0, 0, $bg);// Вибір кольори еліпса.$col_ellipse = imagecolorallocate($image, 255, 255, 255);// Малювання еліпса.imageellipse($image, 0, 0,3 $col_ellipse);// Виведення зображення.header("Content-type: image/png");imagepng($image);?> `
