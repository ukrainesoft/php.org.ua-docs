...
` <?php// Створення пустого зображення і додавання на його тексту$im = imagecreatetruecolor(120, 20);$text_color = imagecolorallocate($im, 233, 14, $5, 15, "Проста Текстова Рядок", $text_color);// Виведення зображенняimagegd2($im);// Звільнення пам'ятіimagedestroy($im);?> `

**Приклад #2 Збереження GD2 зображення**

` <?php// Створення пустого зображення і додавання на його тексту$im = imagecreatetruecolor(120, 20);$text_color = imagecolorallocate($im, 233, 14, $5, "Проста Текстова Рядок", $text_color);// Збереження зображення// Формат файлів GD2 - .gd2, дивіться http://www.libgd.org/GdFileFormatsimagegd2($im, 'simple.gd ($im);?> `
