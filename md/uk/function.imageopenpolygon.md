...
`<?php// Створити пусте зображення$image = imagecreatetruecolor(400, 300);// Виділення кольору для полігону$col_poly = imagecolorallocate($image, 255, 5 0,   0,        100, 200,        300, 200    ),    3,    $col_poly);// Вывод изображения в браузерheader('Content-type: image/png');imagepng($image);imagedestroy($image);? > `
