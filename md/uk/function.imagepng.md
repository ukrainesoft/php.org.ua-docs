...
Дозволяє зменшити розмір файлу PNG. Це бітова маска, значенням якої може бути комбінація констант `PNG_FILTER_XXX`. Для увімкнення або вимкнення всіх фільтрів зручно скористатися константами \*\*`PNG_NO_FILTER`**или**`PNG_ALL_FILTERS`\*\*соответственно. Значение по умолчанию (`-1`) відключає фільтрацію.
