...
`<?php// Створення порожнього зображення$image = imagecreatetruecolor(400, 300);// Створення кольору полігону$col_poly = imagecolorallocate($image, 255, 5                    ??? ,   0,        100, 200,        300, 200    ),    3,    $col_poly);// Вывод картинки в броузерheader('Content-type: image/png');imagepng($image);imagedestroy($image);?> `
