...
` <?php// Завантаження зовнішнього зображення $ zend = = imagecreatefromgif ( '. / Zend.gif '); ;// Заливкаimagefilledrectangle($im, 0, 0, 199, 199, IMG_COLOR_TILED);// Виведення картинки в броузерheader('Content-Type: image/png');imagepng($des$y;image ($ zend);?> `
