...
`$filename`, ?int `$foreground_color` = **`null`**): bool
...
`<?php// Створення пустого зображення і додавання тексту$im = imagecreatetruecolor(120, 20);$text_color = imagecolorallocate($im, 233, 14, 91); string текстова рядок', $text_color);// Збереження зображенняimagexbm($im, 'simpletext.xbm');// Звільнення пам'ятіimagedestroy($im);?> `

**Приклад #2 Збереження XBM файлу з відмінним кольором верхнього шару**

`<?php// Створення пустого зображення і додавання тексту$im = imagecreatetruecolor(120, 20);$text_color = imagecolorallocate($im, 233, 14, 91); string текстовий рядок, $text_color); ?> `
