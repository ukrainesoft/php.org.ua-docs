...
`filename` (без розширення .xbm) також використовується як ідентифікатор C XBM, при цьому символи, що не є у поточній локалі цифрами або літерами, замінюються на підкреслення. Якщо `filename`задан как\*\*`null`\*\* `image` буде використано для створення ідентифікатора C.
