...
` <?php$mbox = imap_open("{imap.example.org}INBOX", "username", "password")    or die("Не удалося підключитися: " . imap_last_error());$check box );echo "Повідомлення до позначки для віддалення: " . $check->Nmsgs . "<br />
";imap_delete($mbox, 1);$check = imap_mailboxmsginfo($mbox);echo "Повідомлення після позначки для віддалення: " . .$check->Nmsgs . "<br />
";imap_expunge($mbox);$check = imap_mailboxmsginfo($mbox);echo "Повідомлення після віддалення: " . $check->Nmsgs . "<br />
";imap_close($mbox);?> `
