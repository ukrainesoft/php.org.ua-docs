...
Коли файл вмикається, його код успадковує ту ж [область видимості змінних](language.variables.scope.md), що й рядок, на якому
...
` vars.php<?php$color = 'зелене';$fruit = 'яблуко';?>test.php<?phpecho "Одне $color $fruit"; // Одноinclude 'vars.php';echo "Одно $color $fruit"; // Одне зелене яблуко?> `
...
[коректні теги початку та кінця PHP-коду](language.basic-syntax.phpmode.md).

Якщо "[обгортки URL include](filesystem.configuration.md#ini.allow-url-include)" включені
в PHP, ви можете також вказати файл для увімкнення через URL (за допомогою
HTTP або інших обробників, що підтримуються - дивіться [Підтримувані протоколи та обгортки](wrappers.md) для списку протоколів) замість
...
`<?php/* У цьому прикладі передбачається, що www.example.com налаштований на обробку .php* файлів, але не .txt. Також, 'Спрацює' позначає, змінні* $foo і $bar доступні всередині включається файла. */// Не спрацює; file.txt не обробляється www.example.com як PHPinclude 'http://www.example.com/file.txt?foo=1&bar=2';// Не спрацює; буде шукати файл? file.php?foo=1&bar=2';?> `
...
виведення віддаленого файлу не містить [коректні теги початку та кінця PHP коду](language.basic-syntax.phpmode.md) (так само, як і локальний
...
` <?php// не спрацює, інтерпретується якinclude(('vars.php') == TRUE), є include('1')if (include('vars.php') == TRUE) {    OK';}// спрацюєif ((include 'vars.php') == TRUE) {   echo 'OK';}?> `

**Приклад #5 Вирази `include` та [return](function.return.md)**

` return.php<?php$var=='PHP';return $var;?>noreturn.php<?php$var = 'PHP';?>testreturns.php<?php$foo = include 'return.php' ;echo $foo; // виведе 'PHP'$bar = include 'noreturn.php';echo $bar; // виведе 1?> `
