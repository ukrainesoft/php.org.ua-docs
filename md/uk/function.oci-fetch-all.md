...
**Пример #1 Пример использования**oci\_fetch\_all()\*\*\*\*
...
**Пример #2 Пример использования**oci\_fetch\_all()**с**`OCI_FETCHSTATEMENT_BY_ROW`\*\*\*\*
...
**Пример #3 Пример использования**oci\_fetch\_all()**с**`OCI_NUM`\*\*\*\*
...
> **Зауваження** :
> 
> Использование`offset` неефективно. Всі ряди, що пропускаються, включаються в результат запиту повертається базою даних до PHP. Після цього вони виключаються. Більш ефективно використовувати SQL для відступу та обмеження рядів у запиті. Дивіться [oci\_fetch\_array()](function.oci-fetch-array.md)для примеров.
