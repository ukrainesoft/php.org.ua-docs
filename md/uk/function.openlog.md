...
Функция**openlog()** відкриває з'єднання до системного журналу програми.

Использование**openlog()** не є обов'язковим. Ця функція за необхідності буде викликана функцією [syslog()](function.syslog.md) автоматично, при цьому параметр `prefix`будет по умолчанию равен\*\*`false`\*\*

### Список параметрів

`prefix`

Рядок `prefix`добавляется к каждому сообщению.
