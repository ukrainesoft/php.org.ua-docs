...
` <?php$ciphers             = openssl_get_cipher_methods();$ciphers_and_aliases = openssl_get_cipher_methods(true);$cipher_aliases      = array_diff($ciphers_and_aliases, $ciphers);//Режим ECB следует избегать$ciphers = array_filter( $ciphers, function($n) { return stripos($n,"ecb")===FALSE; } );//По крайньому мірі, на початку серпня 2016 року наступні методи оголошені слабкими:$ciphers =      stripos($c,"des")===FALSE; } );$ciphers = array_filter( $ciphers, function($c) { return stripos($c,"rc2")===FALSE; } );$ ciphers = array_filter( $ciphers, function($c) { return stripos($c,"rc4")===FALSE; } );$ciphers = array_filter( $ciphers, function($c)$ | ,"md5")===FALSE; } );$cipher_aliases = array_filter($cipher_aliases,function($c) { return stripos($c,"des")===FALSE; } );$cipher_alies $cipher_aliases,function($c) { return stripos($c,"rc2")===FALSE; } );print_r($ciphers);print_r($cipher_aliases);?> `
