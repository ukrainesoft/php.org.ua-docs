...
`<?php// Повідомлення, ви хочете зашифрувати і надіслати своєму секретному// агенту з позивним Козодою. У вас є сертифікат у файлі nighthawk.pem $ data ==<<<EODКозодою,Цілком! Після прочитання спалити! Вороги наближаються! Зв'язковий з новим підробленим паспортомбуде ждати тебе завтра в кафе в 8.30 ранку.Пароль - "У вас ус відклеївся! );// записуємо повідомлення в файл$fp = fopen("msg.txt", "w");fwrite($fp, $data);fclose($fp);// шифруємоif (openssl_pkcs7" , "enc.txt", $key,    array("To" => "nighthawk@example.com", // ассоциативный синтаксис          "From: HQ <hq@example.com>", // индексированный синтаксис          "Subject" = > "ТЕРМІНОВО!!! ВАЖЛИВО!!!"))) {    // повідомлення зашифроване - відправляємо!! exec(ini_get("sendmail_path") . " < enc.txt");}?> `
