...
` <?php$str = "
";if (ord($str) == 10) {    echo "Перший символ \$str - переклад рядки.
";}?> `

**Приклад #2 Перегляд індивідуальних байтів рядка UTF-8**

` <?phpdeclare(encoding='UTF-8');$str = "🐘";for ( $pos=0; $pos < strlen($str); $pos ++ ) { $byte = substr($str , $pos); echo 'Байт ' . $pos . ' рядки $str рівний ' . ord($byte) . PHP_EOL;}?> `
