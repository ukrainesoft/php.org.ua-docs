...
Укажите одну из констант\*\*`PHP_URL_SCHEME`\*\* **`PHP_URL_HOST`** **`PHP_URL_PORT`** **`PHP_URL_USER`** **`PHP_URL_PASS`** **`PHP_URL_PATH`** **`PHP_URL_QUERY`**или**`PHP_URL_FRAGMENT`**, щоб отримати лише конкретний компонент URL у вигляді рядка (string). Винятком є ​​вказівка **`PHP_URL_PORT`**, в цьому випадку значення, що повертається буде типу int.
...
**Пример #1 Пример использования**parse\_url()\*\*\*\*
...
**Пример #2 Пример использования**parse\_url()**при отсутствии протокола**
