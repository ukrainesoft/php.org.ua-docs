...
-   **`PASSWORD_BCRYPT`** - використовує алгоритм **`CRYPT_BLOWFISH`**. Генерує стандартний хеш, сумісний із генерованою функцією [crypt()](function.crypt.html) із використанням ідентифікатора "$2y$". В результаті буде згенеровано рядок довжиною 60 символів або **`false`** у разі виникнення помилки.
...
-   `cost` (int) - ставить необхідну алгоритмічну складність. Приклад використання цього значення можна переглянути на сторінці присвяченої функції [crypt()](function.crypt.html)
...
Асоціативний масив із опціями. За документацією щодо підтримуваних опцій для кожного алгоритму зверніться до розділу [Константи алгоритмів хешування паролів](password.constants.html)
