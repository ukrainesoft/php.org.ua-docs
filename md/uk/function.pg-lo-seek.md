...
`<?php  $doc_oid==189762345; $database==pg_connect("dbname=jacarta"); pg_query($database, "begin"); $handle = pg_lo_open($database, $doc_oid, "r"); // Пропустити перші 50000 байт   pg_lo_seek($handle, 50000, PGSQL_SEEK_SET); // Прочитати наступні 10000байт   $data = pg_lo_read($handle, 10000); pg_query($database, "commit"); echo $data;?> `
