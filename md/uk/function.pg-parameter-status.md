...
`$connection` = ?, string `$param_name`): string
...
` <?php  $dbconn = pg_connect("dbname=publisher") or die("Неможливо з'єднатися з сервером"); echo "Кодування сервера: ", pg_parameter_status($dbconn, "server_encoding");?> `
