...
**пгqueryparams()** подібна до функції [pg\_query()](function.pg-query.html), але надає додатковий функціонал: параметри запиту можна надсилати окремо від рядка запиту . **пгqueryparams()** підтримується на з'єднаннях із серверами PostgreSQL версій 7.4 та вище. Функція не працюватиме із серверами ранніх версій.
...
Головна перевага **пгqueryparams()** перед [pg\_query()](function.pg-query.html) полягає в тому, що значення параметрів можуть передаватися окремо від рядка запиту `query`. Це дає можливість уникнути стомлюючу і схильну до помилок процедуру екранування спецсимволів і укладання значень у лапки. Однак, на відміну від [pg\_query()](function.pg-query.html), ця функція підтримує лише один SQL запит у рядку, що передається. (Він може містити крапку з комою, але не більше однієї непустої SQL-команди.)
...
Користувацькі дані завжди повинні передаватися як параметри, і не передаватися в рядок запиту безпосередньо, де вони можуть призвести до можливих атак через [SQL-инъекции](security.database.sql-injection.html) і призводить до помилок, якщо дані містять лапки. Якщо з якихось причин ви не можете використовувати параметр, переконайтеся, що дані користувача [правильно экранированы](function.pg-escape-string.html)
...
Значення призначені для `bytea` полів не можна передавати у параметрах. Використовуйте функцію [pg\_escape\_bytea()](function.pg-escape-bytea.html) або функції великих об'єктів.
