...
Якщо заданий аргумент `flags`, то до масиву `conditions` буде застосовано функцію [pg\_convert()](function.pg-convert.html) з параметрами, заданими як аргумент.

За замовчуванням [pg\_insert()](function.pg-insert.html) передає необроблені значення. Значення мають бути екрановані або опція PGSQLDMLESCAPE має бути вказана. PGSQLDMLESCAPE містить лапки і екранує параметри/ідентифікатори. Тому імена таблиць/стовпців стають чутливими до регістру.
...
Одна з констант **`PGSQL_CONV_FORCE_NULL`** **`PGSQL_DML_NO_CONV`** **`PGSQL_DML_ESCAPE`** **`PGSQL_DML_EXEC`** **`PGSQL_DML_ASYNC`** **`PGSQL_DML_STRING`** чи їх комбінація. Якщо `flags` містить **`PGSQL_DML_STRING`**, функція поверне рядок. Якщо встановлено **`PGSQL_DML_NO_CONV`** або **`PGSQL_DML_ESCAPE`**, то функція [pg\_convert()](function.pg-convert.html) внутрішньо не викликається.
