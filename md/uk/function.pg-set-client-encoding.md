...
`$connection` = ?, string `$encoding`): int
...
`LATINX` (X=1...9), `KOI8`, `WIN`, `ALT`, `SJIS`, `BIG5` or `WIN1250`.
...
";  exit;}// Виводимо UTF-8 даніwhile ($row = pg_fetch_row($result)) {  echo "Author: $row[0]  E-mail: $row[1]";    
";}?> `
