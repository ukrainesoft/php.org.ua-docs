...
` <?php  $dbconn = pg_connect("dbname=publisher") or die("Підключитися не вдалося"); $stat = pg_transaction_status($dbconn); if ($stat === PGSQL_TRANSACTION_UNKNOWN) {      echo 'З'єднання не удалося'; } else if ($stat === PGSQL_TRANSACTION_IDLE) {     echo 'Сполука вільно і простує'; } else {      echo 'З'єднання в режимі транзакції'; }?> `
