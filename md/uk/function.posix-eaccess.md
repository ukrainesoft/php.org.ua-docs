...
Маска, що складається з однієї або кількох констант: **`POSIX_F_OK`** **`POSIX_R_OK`** **`POSIX_W_OK`**и**`POSIX_X_OK`**

Константи **`POSIX_R_OK`** **`POSIX_W_OK`**и**`POSIX_X_OK`** запитують перевірку існування файлу та наявності дозволів на читання, запис та виконання відповідно. Константа **`POSIX_F_OK`** просто запитує перевірку існування файлу.
...
**Пример #1 Пример использования функции**posix\_eaccess()\*\*\*\*
