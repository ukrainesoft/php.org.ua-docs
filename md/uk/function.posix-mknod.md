...
Цей параметр виходить за допомогою побітового АБО між типом файлу (однієї з наступних констант: **`POSIX_S_IFREG`** **`POSIX_S_IFCHR`** **`POSIX_S_IFBLK`** **`POSIX_S_IFIFO`**или**`POSIX_S_IFSOCK`**) та правами доступу.

`major`

Старший номер пристрою (обов'язковий параметр під час використання констант **`S_IFCHR`**или**`S_IFBLK`**
...
**Пример #1 Пример использования**posix\_mknod()\*\*\*\*
