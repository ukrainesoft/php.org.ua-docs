Завершити процес, відкритий procopen та повернути код повернення цього процесу

-   [« passthru](function.passthru.html)
    
-   [proc\_get\_status »](function.proc-get-status.html)
    
-   [PHP Manual](index.html)
    
-   [Функции запуска программ](ref.exec.html)
    
-   Завершити процес, відкритий procopen та повернути код повернення цього процесу
    

# procclose

(PHP 4> = 4.3.0, PHP 5, PHP 7, PHP 8)

procclose — Завершити процес, відкритий [proc\_open()](function.proc-open.html) та повернути код повернення цього процесу

### Опис

```methodsynopsis
proc_close(resource $process): int
```

Функція **procclose()** схожа на функцію [pclose()](function.pclose.html), за винятком того, що вона працює тільки з процесами, відкритими за допомогою функції [proc\_open()](function.proc-open.html). Функція **procclose()** очікує завершення процесу та повертає його код повернення. Відкриті канали для цього процесу закриваються під час виклику цієї функції, щоб уникнути повної зупинки програми: дочірній процес не зможе завершитись, якщо є відкриті канали.

### Список параметрів

`process`

Дескриптор типу resource, відкритий за допомогою [proc\_open()](function.proc-open.html), що буде закрито.

### Значення, що повертаються

Повертає код завершення процесу, який було запущено. У разі помилки повертається `-1`

> **Зауваження**
> 
> Якщо PHP зібрано з опцією --enable-sigchild, значення цієї функції, що повертається, не визначено.