...
` <?php$value = 'щось звідки-то';setcookie("TestCookie", $value);setcookie("TestCookie", $value, time()+3600); /* термін дії 1 година */setcookie("TestCookie", $value, time()+3600, "/~rasmus/", "example.com", 1);?> `
...
` <?php// відправка cookiesetcookie("cookie[three]", "cookiethree");setcookie("cookie[two]", "cookietwo");setcookie("cookie[one]", "cookieone");/ / після перезавантаження сторінки, виведемо cookieif (isset($_COOKIE['cookie'])) {    foreach ($_COOKIE['cookie'] as $name => $value)    $value==htmlspecialchars($value); echo "$name : $value <br />
";    }}?> `
