...
Повертає частину значення, закодовану у параметрі `json` на яку посилається ключ `key` у відповідному PHP-типі. Значення `true` `false`и`null` повертаються як **`true`** \*\*`false`**и**`null`\*\*соответственно.
