...
Якщо заданий сокет має тип **`AF_INET`**или**`AF_INET6`** **socket\_getpeername()** поверне віддалений *IP-адреса* у відповідному форматі (наприклад, `127.0.0.1`или`fe80::1`) в параметре`address` і якщо необов'язковий параметр `port` є також пов'язаний порт.
...
Повертає **`true`** у разі успішного виконання або **`false`**в случае возникновения ошибки**socket\_getpeername()** також може повернути \*\*`false`\*\*якщо сокет мати тип відмінний від **`AF_INET`** **`AF_INET6`**или**`AF_UNIX`**, у цьому випадку код останньої помилки сокету *НЕ* буде оновлено.
