...
Якщо заданий сокет має тип **`AF_INET`** або **`AF_INET6`** [socketgetpeername()](function.socket-getpeername.md) поверне локальний *IP-адреса* у відповідному форматі (наприклад, `127.0.0.1` або `fe80::1`) у параметрі `address` і якщо необов'язковий параметр `port` є також пов'язаний порт.

Якщо заданий сокет має тип **`AF_UNIX`** [socketgetpeername()](function.socket-getpeername.md) поверне шлях у файловій системі Unix (тобто . `/var/run/daemon.sock`) у параметр `address`
