...
` <?php$socket = socket_create(AF_INET, SOCK_DGRAM, SOL_UDP);socket_bind($socket, '127.0.0.1', 1223);$from = '';$port = 0;so$_rec , 0, $from, $port);echo "Отримано $buf з віддаленої адреси $from і віддаленого порту $port" . PHP_EOL;?> `
