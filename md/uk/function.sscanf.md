...
Функция\*\*sscanf()\*\*похожа на функцию[printf()](function.printf.md), але використовується не для виведення, а для введення даних . **sscanf()** інтерпретує рядок `string` відповідно до формату `format`
...
Якщо очікується більше підрядків у параметрі `format`, чем доступно в параметре`str`, то буде повернено значення **`null`**
...
**Пример #2**sscanf()\*\* - Використання необов'язкових аргументів\*\*
