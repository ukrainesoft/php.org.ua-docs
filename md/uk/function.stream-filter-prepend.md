...
> **Зауваження**: Коли фільтр додається для читання та запису, створюються два екземпляри фільтра. Функція [stream\_filter\_append()](function.stream-filter-append.md) повинна бути викликана двічі з **`STREAM_FILTER_READ`**и**`STREAM_FILTER_WRITE`** щоб отримати обидва ресурси фільтра.
