...
Схожа на функцію [filegetcontents()](function.file-get-contents.md), за винятком того, що **streamgetcontents()** працює з уже відкритим ресурсом потоку і повертає решту вмісту в рядок розміром до `length` байт і починаючи із зазначеного зміщення `offset`
