...
` <?php$server = stream_socket_server('tcp://127.0.0.1:1337');$client = stream_socket_client('tcp://127.0.0.1:1337');var_dump(fputs($client, ));stream_socket_shutdown($client, STREAM_SHUT_WR);var_dump(fputs($client, "привіт")); // не працює зараз?> `

Результатом виконання цього прикладу буде щось подібне:

int(5)

Notice: fputs(): send of 6 bytes failed with errno=32 Перекинутий pipe in test.php on line 9
int(0)
