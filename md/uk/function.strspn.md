...
Повертає довжину ділянки від початку рядка `string`, содержащий*тільки* символи з `characters`

Якщо параметри `offset`и`length` не вказано, то буде досліджено весь рядок `string`. Якщо вони вказані, то ефект буде аналогічним виклику `strspn(substr($string, $offset, $length), $characters)`(подробнее смотрите функцию[substr](function.substr.md)
...
присвоїть змінної $var, тому що "42" - це початкова ділянка рядка `string`, Що складається тільки з символів "1234567890".
...
Якщо `offset` вказаний і невід'ємний, то **strspn()** почне шукати у рядку `string`, починаючи з позиції `offset`. . Наприклад, у рядку '`abcdef`', символ з позиції є '`a`', символ з позиції є '`c`' і т.д.
...
Якщо `length` вказаний і невід'ємний, то рядок `string`будет исследована в течение`length`после стартовой позиции.
...
Возвращает длину первого участка`string`, що складається цілком із символів у `characters`

> **Зауваження** :
> 
> Когда`offset` вказано, довжина рядка, що повертається, визначається, починаючи з цієї позиції, а не з початку рядка `string`
