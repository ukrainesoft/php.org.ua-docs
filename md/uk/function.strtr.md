...
може бути масивом (array), у вигляді `array('from' => 'to', ...)`.
...
` <?php//В цій формі strtr() здійснює побайтне перетворення//Таким образом, тут мається на увазі однобайтне кодування:$addr = strtr($addr, "äåö", ;);
