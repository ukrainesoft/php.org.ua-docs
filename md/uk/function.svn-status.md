...
svn\_status — Повертає SVN-статус файлів та директорій робочої копії
...
Будь-яка комбінація прапорів **`Svn::NON_RECURSIVE`** **`Svn::ALL`**(вне зависимости от статуса модификации),**`Svn::SHOW_UPDATES`** (до записів будуть додані пункти зі старими правками), **`Svn::NO_IGNORE`**(игнорирование`svn:ignore` властивостей при скануванні нових файлів) та **`Svn::IGNORE_EXTERNALS`**
...
Код текстового статусу елемент. Дивіться Для інформації про можливі значення, зверніться до [константам статусів](svn.constants.md#svn.constants.status)

repos\_text\_status

Код текстового статусу елемента у репозиторії. Відображається якщо `update`установлен в\*\*`true`\*\*. Дивіться Для інформації про можливі значення, зверніться до [константам статусів](svn.constants.md#svn.constants.status)

prop\_status

Код текстового статусу властивостей елемента. Дивіться Для інформації про можливі значення, зверніться до [константам статусів](svn.constants.md#svn.constants.status)

repos\_prop\_status

Код текстового статусу властивостей елемента у репозиторії. Відображається якщо `update`установлен в\*\*`true`\*\*. Дивіться Для інформації про можливі значення, зверніться до [константам статусів](svn.constants.md#svn.constants.status)
...
Тип елемента, тобто. файл чи директорія. Дивіться Для інформації про можливі значення, зверніться до [константам типів](svn.constants.md#svn.constants.type)
...
-   [» SVN-документація з svn status](http://svnbook.red-bean.com/en/1.2/svn.ref.svn.c.status.md)
