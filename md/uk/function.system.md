...
`<?phpecho '<pre>';// Виводить весь результат команди оболонки "ls" і повертає// останнє рядок висновку в змінної $last_line. Зберігає код повернення// команди в $retval.$last_line = system('ls', $retval);// Виводимо додаткову інформаціюecho '</pre><hr />Остання рядок висновку: ' $last_line. '<hr />Код повернення: ' . $retval;?> `
