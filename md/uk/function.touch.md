...
**touch**(string `$filename`, ?int `$mtime` = **`null`**, ?int `$atime`
= **`null`**): bool
...
` <?php// Це час дотику, встановимо його на годину назад.$time = time() - 3600;// Змінюємо файлif (!touch('some_file.txt''        то пішло не так...';} else {   echo 'Зміна модифікації файлу пройшло успішно';}?> `
