Затримка виконання у мікросекундах

-   [« unpack](function.unpack.html)
    
-   [Список изменений »](changelog.misc.html)
    
-   [PHP Manual](index.html)
    
-   [Різні функції](ref.misc.html)
    
-   Затримка виконання у мікросекундах
    

# usleep

(PHP 4, PHP 5, PHP 7, PHP 8)

usleep — Затримка виконання у мікросекундах

### Опис

```methodsynopsis
usleep(int $microseconds): void
```

Відкладає виконання програми на вказану кількість мікросекунд.

### Список параметрів

`microseconds`

Час виконання, що відкладається в мікросекундах. Мікросекунда – це одна мільйонна секунди.

> **Зауваження**: Значення більше `1000000` (тобто очікування більше секунди) можуть не підтримуватись операційною системою. Натомість використовуйте [sleep()](function.sleep.html)

> **Зауваження**: Відкладене виконання може бути трохи довшим (тобто може бути довшим, ніж параметр `microseconds`) за рахунок будь-якої активності системи або часу, витраченого на обробку виклику або гранулярністю системних таймерів.

### Значення, що повертаються

Функція не повертає значення після виконання.

### Приклади

**Приклад #1 Приклад використання **usleep()****

```php
<?php

// Текущее время
echo date('h:i:s') . "\n";

// ждать 2 секунды
usleep(2000000);

// вернуться к исполнению
echo date('h:i:s') . "\n";

?>
```

Результат виконання цього прикладу:

```
11:13:28
11:13:30
```

### Дивіться також

-   [sleep()](function.sleep.html) - затримка виконання
-   [timenanosleep()](function.time-nanosleep.html) - Затримка на задане число секунд та наносекунд
-   [timesleepuntil()](function.time-sleep-until.html) - Відкладає виконання скрипту до заданого часу
-   [settimelimit()](function.set-time-limit.html) - Обмеження часу виконання скрипту