...
`будь-який рядок, не знайдений у цьому списку` \< `dev` \< `alpha` = `a` \<
`beta` = `b` \< `RC` = `rc` \< `#` \< `pl` = `p`. Таким чином можна
...
`operator`
Необов'язковий параметр `operator`. Можливі значення: `<`, `lt`, `<=`,
`le`, `>`, `gt`, `>=`, `ge`, `==`, `=`, `eq`, `!=`, `<>`, `ne`.
...
` <?phpif (version_compare(PHP_VERSION, '7.0.0') >= 0) {   echo 'Я використовую PHP версії не нижче 7.0.0, моя версія: ' . PHP_VERSION . "
";}if (version_compare(PHP_VERSION, '5.3.0') >= 0) {   echo 'Я використовую PHP версії не нижче 5.3.0, моя версія: ' . .
";}if (version_compare(PHP_VERSION, '5.0.0', '>=')) {    echo 'Я використовую PHP 5.0.0 або вище, моя версія: ' . P 
