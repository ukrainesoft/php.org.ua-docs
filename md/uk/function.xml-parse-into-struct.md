...
title: xml\_parse\_into\_struct
...
**xml\_parse\_into\_struct()** повертає 0 при невдалому розборі рядка та 1 при успішному. Це не те саме, що **`false`**и**`true`**, будьте обережні з такими операторами, як ===.
...
**Пример #1 Пример использования**xml\_parse\_into\_struct()\*\*\*\*
