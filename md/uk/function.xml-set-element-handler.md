...
Третій аргумент `attribs` містить асоціативний масив з атрибутами елемента (якщо є). Індексами цього масиву будуть імена атрибутів, а значення масиву відповідатимуть значенням атрибутів. Імена атрибутів мають бути [приведены к регистру](xml.case-folding.html) відповідно до тих самих критеріїв, що і імена елементів. Значення атрибутів *не* наводяться до якогось регістру. Атрибути витягуються з елемента в порядку прямого обходу `attribs`, використовуючи функцію [each()](function.each.html). Перший індекс у масиві відповідає першому атрибуту тощо.
