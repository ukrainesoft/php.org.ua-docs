...
Анонімні функції, також відомі як замикання (`closures`), дозволяють створювати функції, що не мають певних імен. Вони найбільш корисні як значення [callable](language.types.callable.html)параметрів, але також можуть мати безліч інших застосувань.

Анонімні функції реалізуються з використанням класу [](class.closure.html)[Closure](class.closure.html)
...
Замикання також можуть бути використані як значення змінних; PHP автоматично перетворює такі вирази на екземпляри внутрішнього класу [Closure](class.closure.html). Присвоєння замикання змінної використовує той самий синтаксис, що і для будь-якого іншого присвоєння, включаючи точку з комою:
...
Замикання можуть успадковувати змінні з батьківської області видимості. Будь-яка подібна змінна має бути оголошена в конструкції `use`. Починаючи з PHP 7.1, ці змінні не повинні включати [superglobals](language.variables.predefined.html), $this і змінні з тими самими іменами, як і параметри функції. Оголошення типу значення функції, що повертається, повинно бути поміщене *після* конструкції `use`
...
| Версия | Описание                                                                                                                                                   |
|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
|        | Анонімні функції не можуть замикатися довкола [superglobals](language.variables.predefined.html), $this або будь-яка змінна з тим же ім'ям, що і параметр. |
