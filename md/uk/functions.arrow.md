...
Стрілецькі функції підтримують ті ж можливості, що і [анонімні функції](functions.anonymous.md), за винятком того, що
...
` <?php$y = 1;$fn1 = fn($x) => $x + $y;// еквівалентно використання $y за призначенням:$fn2 = function ($x) use ($y x + $y;};var_export($fn1(3));?> `
...
` <?php$z = 1;$fn = fn($x) => fn($y) => $x * $y + $z;// Виведе 51var_export($fn(5)(10)); ?> `
...
` <?php$x = 1;$fn = fn() => $x++; // Нічого не змінить$fn();var_export($x); // Виведе 1?> `
