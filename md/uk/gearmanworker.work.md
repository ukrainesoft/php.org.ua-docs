...
` <?php# створюємо обробник$worker = new GearmanWorker();# додаємо сервер задань за замовчуванням (localhost)$worker->addServer();# додаємо callback-функцію$ );# запускаємо обробник, очікує задань від сервераwhile ($worker->work());function my_reverse_function($job){ return strrev($job->workload());}?> `
