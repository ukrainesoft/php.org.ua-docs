...
Чекає від сервера завдання, а потім викликає відповідну callback-функцію для обробки. Викликає помилку рівня **`E_WARNING`**с информацией о последней ошибке Gearman в случаях, когда код возврата функции отличается от**`GEARMAN_SUCCESS`** **`GEARMAN_IO_WAIT`**или**`GEARMAN_WORK_FAIL`**
