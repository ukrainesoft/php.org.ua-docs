...
`<?phpheader('Content-type: image/jpeg');$image = new Imagick('image.jpg');// Якщо в якості ширини або висоти переданий 0,// то зберігається співвідношення> thumbnailImage(100, 0);echo $image;?> `
...
` <?php$images = new Imagick(glob('images/*.JPG'));foreach($images as $image) {    // Передаєм 0 в thumbnailImage для збереження спів$ );}$images->writeImages();?> `
