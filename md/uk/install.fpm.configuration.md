---
navigation:
  - install.fpm.install.html: « Установка
...
Увімкніть оформлення виходу (output decoration) для виведення worker-процесу коли опція [catchworkersoutput](install.fpm.configuration.html#catch-workers-output) включено. Значення за промовчанням: yes. Доступно з PHP 7.3.0.
...
Оскільки ці налаштування передаються в php-fpm як FastCGI-заголовки, php-fpm не повинен бути прив'язаний до загальнодоступної адреси світу. В іншому випадку будь-хто зможе змінити налаштування PHP. Дивіться також [listen.allowedclients](install.fpm.configuration.html#listen-allowed-clients)
