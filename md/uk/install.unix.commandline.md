...
Деякі [переменные окружения сервера](reserved.variables.server.html) не визначено у поточній [» спецификации CGI/1.1](http://www.faqs.org/rfcs/rfc3875). Визначено лише такі змінні: AUTHTYPE, CONTENTLENGTH, CONTENTTYPE, GATEWAYINTERFACE, PATHINFO, PATHTRANSLATED, QUERYSTRING, REMOTEADDR, REMOTEHOST, REMOTEIDENT, REMOTEUSER, REQUESTMETHOD, SCRIPTNAME, SERVERNAME, SERVERPORT, SERVERPROTOCOL та SERVERSOFTWARE. Решта має оброблятися як додаткові модулі (vendor extensions).