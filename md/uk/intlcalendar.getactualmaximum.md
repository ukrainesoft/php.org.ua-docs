...
Повертає відносне значення поля для поточного часу. Точна семантика залежить від поля, але в загальному випадку це значення, яке було б отримано, якщо встановити значення поля на [наименьший относительный максимум](intlcalendar.getleastmaximum.html) і збільшувати його до тих пір, поки не буде досягнуто [глобальный максимум](intlcalendar.getmaximum.html), щоб обернути значення поля, в якому значення, що повертається буде глобальним максимумом або значенням до перенесення, відповідно.
