...
Однією із значних переваг PHP є підтримка [широкого круга баз данных](refs.database.html). Створити скрипт, який використовує бази даних, неймовірно просто. Можна скористатися модулем, специфічним для окремої бази даних (такі як [mysql](book.mysqli.html)) або використовувати рівень абстракції від бази даних, такий як [PDO](book.pdo.html), або приєднатися до будь-якої бази даних, що підтримує Відкритий Стандарт З'єднання Баз Даних (ODBC), за допомогою однойменного модуля [ODBC](book.uodbc.html). Для інших баз даних, таких як CouchDB, можна скористатися [cURL](book.curl.html) або [сокетами](book.sockets.html)
...
PHP має багато можливостей з [обработке текста](refs.basic.text.html), включаючи регулярні вирази Perl ([PCRE](book.pcre.html)) та багато інших модулів та інструментів [для обработки и доступа к XML-документам](refs.xml.html). У PHP обробка XML-документів стандартизована та відбувається на базі потужної бібліотеки [libxml2](book.libxml.html), розширивши можливості обробки XML додаванням нових модулів [SimpleXML](book.simplexml.html) [XMLReader](book.xmlreader.html) і [XMLWriter](book.xmlwriter.html)

Є ще багато інших цікавих модулів, які можна переглянути як у [алфавітному порядку](extensions.html), так і [по категориям](funcref.html). Є ще багато додаткових модулів PECL, які також можуть (а можуть і ні) бути документовані у цьому посібнику, такі як [» XDebug](http://xdebug.org/)

Як ви бачите, цієї сторінки не вистачить, щоб перерахувати все, що може запропонувати вам PHP. Читайте наступний розділ, [Установка PHP](install.html) і зверніться до глави [Справочник по функциям](funcref.html) за більш детальними відомостями про перераховані вище модулі.