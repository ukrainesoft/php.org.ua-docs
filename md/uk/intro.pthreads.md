...
Спробуйте замість цього використати [parallel](book.parallel.html)
...
Клас [Threaded](class.threaded.html) надає базовий функціонал, необхідний для роботи pthreads. Він надає методи синхронізації та деякі корисні для розробника інтерфейси.

Клас [Thread](class.thread.html) дозволяє створювати потоки. Для створення потоку необхідно створити наслідуючий його клас і в ньому реалізувати метод `run`. Будь-які властивості можуть бути записані та прочитані з будь-якого контексту через посилання на потік. Також із будь-якого контексту можуть бути викликані публічні та захищені методи. Метод run буде запущений в окремому потоці, як тільки буде викликаний метод [Thread::start()](thread.start.html) із контексту, в якому він був створений. Стартувати та приєднати потік можна лише у тому контексті, де він був створений.

Клас [Worker](class.worker.html) має постійний стан і буде доступний з методу [Thread::start()](thread.start.html) (успадкований метод) доти, доки об'єкт не вийде з області видимості, або поки не буде примусово зупинений (за допомогою [Worker::shutdown()](worker.shutdown.html)). Будь-який контекст, що має посилання на цей об'єкт, може додавати завдання на стек Worker (за допомогою [Worker::stack()](worker.stack.html)), які будуть запущені в окремому потоці. Метод `run` об'єкта Worker буде запущений раніше, ніж будь-який об'єкт з його стека завдань, що дає можливість ініціалізувати необхідні для завдання ресурси.

Клас [Pool](class.pool.html) використовується для створення групи виконавців (worker) для розподілу між ними об'єктів [Threaded](class.threaded.html). Це найпростіший і найефективніший варіант використання багатопоточності в PHP.

**Застереження**

Клас [Pool](class.pool.html) не успадковує [Threaded](class.threaded.html)таким чином є звичайним об'єктом PHP. Об'єкти цього класу не повинні розділятися за різними контекстами.

Клас [Volatile](class.volatile.html) доданий до pthreads v3. Він використовується для завдання змінюваних [Threaded](class.threaded.html)властивостей класів типу [Threaded](class.threaded.html) (оскільки тепер вони за умовчанням незмінні). Також використовується для зберігання масивів PHP у контексті [Threaded](class.threaded.html)

Синхронізація – важлива частина багатопоточності. Всі об'єкти, створені pthreads, мають вбудовані методи для забезпечення синхронізації (вони здадуться дуже знайомими для розробників Java): [Threaded::wait()](threaded.wait.html) і [Threaded::notify()](threaded.notify.html). Виклик [Threaded::wait()](threaded.wait.html) переведе об'єкт в очікування до того моменту, поки з іншого контексту не буде викликано [Threaded::notify()](threaded.notify.html) для об'єкта, що очікує. Цей механізм дозволяє синхронізувати між об'єктами. [Threaded](class.threaded.html)

**Застереження**

Будь-які об'єкти вашої програми, які передбачається використовувати в багатопотоковому режимі, повинні успадковувати [Threaded](class.threaded.html)
...
Коли запускаються printr, vardump або інші функції налагодження, вони не включають захист від рекурсії.
