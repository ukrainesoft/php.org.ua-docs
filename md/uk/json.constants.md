...
Наступні константи вказують на тип помилки, повернутий функцією [json\_last\_error()](function.json-last-error.html) або зберігається, як code в [JsonException](class.jsonexception.html)
...
Об'єкт або масив, переданий у функцію [json\_encode()](function.json-encode.html) включає рекурсивні посилання та не може бути закодований. Якщо було передано опцію **`JSON_PARTIAL_OUTPUT_ON_ERROR`**, то на місці рекурсивних посилань буде виведено **`null`**

**`JSON_ERROR_INF_OR_NAN`** (int)

Значення, передане у функцію [json\_encode()](function.json-encode.html), включає або [**`NAN`**](language.types.float.html#language.types.float.nan), або [**`INF`**](function.is-infinite.html). Якщо було вказано константу **`JSON_PARTIAL_OUTPUT_ON_ERROR`**, то замість зазначених особливих значень буде виведено `0`

**`JSON_ERROR_UNSUPPORTED_TYPE`** (int)

У функцію [json\_encode()](function.json-encode.html) було передано значення непідтримуваного типу, наприклад, [resource](language.types.resource.html). Якщо було вказано константу **`JSON_PARTIAL_OUTPUT_ON_ERROR`**, то замість непідтримуваного значення виводитиметься **`null`**

**`JSON_ERROR_INVALID_PROPERTY_NAME`** (int)

У рядку переданому в [json\_decode()](function.json-decode.html) був ключ, що починається з символу u0000.

**`JSON_ERROR_UTF16`** (int)

Один непарний сурогат UTF-16 в екранованій послідовності Unicode у рядку JSON, переданому в [json\_decode()](function.json-decode.html)

Можна комбінувати наступні константи для передачі [json\_decode()](function.json-decode.html)
...
Перетворює об'єкти JSON на масив PHP. Ця опція може бути задана автоматично, якщо викликати [json\_decode()](function.json-decode.html) вказавши другим параметром **`true`**

Наступні константи можна комбінувати для використання в [json\_encode()](function.json-encode.html)
...
Наступні константи можна комбінувати для використання в [json\_decode()](function.json-decode.html) і [json\_encode()](function.json-encode.html)
...
Викидається виняток [JsonException](class.jsonexception.html) у разі виникнення помилок замість встановлення глобального стану помилки, який можна отримати за допомогою функції [json\_last\_error()](function.json-last-error.html) і [json\_last\_error\_msg()](function.json-last-error-msg.html). Константа **`JSON_PARTIAL_OUTPUT_ON_ERROR`** має пріоритет над **`JSON_THROW_ON_ERROR`**. Доступно з PHP 7.3.0.