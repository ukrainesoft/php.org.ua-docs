...
записуєте "`$a` = 5", ви привласнюєте '5' змінною `$a`. '5',
...
тому, якщо ви написали `$b` = `$a`, ви вважаєте, що це працювати
буде так само, якби ви написали `$b` = 5. Іншими словами, `$a`
...
що запис `$c = foo()` абсолютно еквівалентний запису `$c = 5`, і ви
...
ми вже мали справу: $a` = 5'. Легко помітити, що тут присутні
...
на практиці це означає, що '`$a` = 5', незалежно від того, що воно
робить, є виразом зі значенням 5. Таким чином, запис '`$b`
= (`$a` = 5)' рівносильна запису '`$a` = 5; `$b` = 5; (крапка з комою
позначає кінець виразу). Оскільки операції присвоєння аналізуються
праворуч наліво, ви можете написати '`$b` = `$a` = 5'.
...
Це [оператори інкременту та декремента](language.operators.increment.md). Так само як і C, PHP
...
операції порівняння > (більше), \>= (більше чи одно), == (рівно), !=
(Не одно), \<(менше) і \<= (менше або одно). Він також підтримує
оператори суворої рівності: === (і одного типу) і !== (не одно
...
є запис виду ``$a` = `$a` + 3'. '`$a` + 3' обчислюється в
...
поточним значенням `$a` можна за допомогою запису ``$a` += 3'. Це означає
...
працює. Значенням '`$a` += 3', як і звичайного присвоєння, є
надане значення. Зверніть увагу, що це НЕ 3, а підсумоване
значення `$a` плюс 3 (те, що було присвоєно `$a`). Таким чином, може
використовуватись будь-який бінарний оператор, наприклад, '`$a` -= 5' (відняти 5
із значення `$a`), '`$b` \*= 7' (помножити значення `$b` на 7) тощо.
...
` <?phpfunction double($i){   return $i*2;}$b = $a = 5; /* присвоїти значення п'ять змінним $a і $b */$c = $a++; /* постфіксний інкремент, присвоїти значення $a                         (5) змінної $c */$e = $d| /* префиксный инкремент, присвоить увеличенное                       значение $b (6) переменным $d и $e *//* в этой точке и $d, и $e равны 6 */$f = double($d++); /* присвоїти подвоєне значення $d перед                        інкрементом (2*6 = 12) змінної $f $| /* присвоїти подвоєне значення $e після                        інкремента (2*7 = 14) змінної $    /* сначала переменная $g увеличивается на 10,                       приобретая, в итоге, значение 24. Затем значение                       присвоения (24) присваивается переменной $h,                       которая в итоге также становится равной 24. */?> `

Деякі вирази можна розглядати як інструкції. В данному
у випадку інструкція має вигляд '`expr ;`' - вираз з наступною точкою
з комою. У записі ``$b = $a = 5;'`, `'$a = 5'` - це вірний вираз,
але саме собою не інструкція. Тоді як ``$b = $a = 5;'' є
вірною інструкцією.
