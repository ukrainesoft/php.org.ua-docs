...
` <?phpfunction gen_one_to_three() {    for ($i = 1; $i <= 3; $i++) {         // Зверніть увага, і yield $i; }}$generator = gen_one_to_three();foreach ($generator as $value) {    echo "$value
";}?> `
...
` <?php/* $input містить пари ключ/значення розділені точкою з|ком| */$input = <<<'EOF'1;PHP;Любит знаки долара2; input) {    foreach (explode("
...
Генератори можуть надавати значення за посиланням. Це робиться так само, як [повернення посилань з функцій](functions.returning-values.md)":
додавання амперсанди (&) перед ім'ям функції.

**Приклад #4 Отримання значень за посиланням**

` <?phpfunction &gen_reference() {    $value = 3; while($value > 0) {       yield $value; }}/* Зверніть увага, що ми можемо змінювати $number в циклі, і * так як генератор повертає посилання, $value * в gen_reference() так. */foreach (gen_reference() as &$number) {   echo (--$number).'... ';}?> `
