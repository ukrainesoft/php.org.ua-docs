...
Значення, яке буде надано $data, - це значення, передане [Generator::send()](generator.send.md) або \*\*`null`\*\*якщо замість цього викликається [Generator::next()](generator.next.md)
...
Генератори можуть надавати значення за посиланням. Це робиться так само, як [повернення посилань із функцій](functions.returning-values.md)": додавання амперсанди (&) перед ім'ям функції.
...
# Збереження масиву (наприклад, за допомогою [iteratorтоarray()](function.iterator-to-array.md)
...
Загальний випадок, коли це має значення, це коли [iteratorтоarray()](function.iterator-to-array.md) повертає масив із ключами за замовчуванням. В цьому випадку можна отримати несподіваний результат . [iteratorтоarray()](function.iterator-to-array.md) має другий параметр `use_keys`, який можна встановити в **`false`**, для генерації власних ключів та ігнорування ключів, переданих з об'єкта [Generator](class.generator.md)

**Приклад #5 **yield from** з [iteratorтоarray()](function.iterator-to-array.md)**
