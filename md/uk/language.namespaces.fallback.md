...
`<?phpnamespace A\B\C;class Exception extends \Exception {}$a = new Exception('hi'); // $a - це об'єкт класу A\B\C\Exception$b = new \Exception('hi'); // $b - це об'єкт класу Exception$c = new ArrayObject; // фатальна помилка, клас A\B\C\ArrayObject не найден?> `
