...
Якщо обидва операнда для `&` `|` і `^` Рядки, то операція буде проводитися з кодами ASCII всіх символів рядка і в результаті поверне рядок. У всіх інших випадках, обидва операнди будуть [перетворені на ціле](language.types.integer.md#language.types.integer.casting) і результатом буде ціле число.
