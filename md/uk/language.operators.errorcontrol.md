...
`<?php// Умисна помилка при роботі з файлами$my_file = @file ('non_existent_file') or   die|("Помилка при відкритті| '");// працює для  будь-яких виразів, а не тільки для функцій$value = @$cache[$key];// У випадку якщо ключа $key ні, повідомлення про >

> **Примітка**: Оператор `@` працює тільки з > [виразами](language.expressions.md). Є просте правило: якщо
