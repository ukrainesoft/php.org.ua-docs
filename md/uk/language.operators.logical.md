...
працюють з різними пріоритетами (див. таблицю [Пріоритет виконання операторів](language.operators.precedence.md)).

**Приклад #1 Пояснення логічних операторів**

` <?php// --------------------// foo() ніколи не буде викликана, т.к. ці оператори є шунтуючими (short-circuit)$a = (false && foo());$b = (true  || foo());$c = (false and foo());$d  ());// --------------------// "||" має більший пріоритет, чому "or"// Результат вираження (false || true) присвоюється змінною $e// Діє як: ($e = (false || true))$ true;// Константа false присвоюється $f, а потім значення true ігнорується// Діє як: (($f = false) or true)$f = false or $ $   ¦¦ ------------------// "&&" має більший пріоритет, чому "and"// Результат вираження (true && false) присвоюється змінною $g// Діє як: ( $g = (true && false))$g = true && false;// Константа true привласнюється $h, а потім значення false ігнорується// false і  ;var_dump($g, $h);?> `
