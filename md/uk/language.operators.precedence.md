...
з іншого боку "=" - право-асоціативний оператор, так що
$a = $b = $c `згрупується як `$a = ($b = $c)`.
...
Вираз `1 <= 1 == 1`, з іншого боку, буде, оскільки `==` має
нижчий пріоритет ніж `<=`.
...
| права          | =+=-=*=**=/=.=%=&= | =^=<<= >>= ??=                                                                                                                                                                                                                                                                                                              | [оператори привласнення](language.operators.assignment.md)                                                                                                                                                                                                                                                                                          
...
` <?php$a = 3 * 3 % 5; // (3 * 3) % 5 = 4// асоціативність тернарних операторів відрізняється від C/C++$a = true ? 0 : true ? 1 : 2; // (true ? 0 : true) ? 1 : 2 = 2 (до PHP 8.0.0)$a = 1;$b = 2;$a = $b += 3; // $a = ($b += 3) -> $a = 5, $b = 5?> `
...
` <?php$a = 1;echo $a + $a++; // може вивести як 2 так і 3$i = 1;$array[$i] = $i++; // може встановити індекс як 1, так 2?> `

**Приклад #3 `+`, `-` та `.` мають однаковий пріоритет (до PHP 8.0.0)**

`<?php$x = 4;// наступний код може видати несподіваний результат:echo "x мінус 1 рівно " . $x-1 . ", ну я сподіваюся
";// оскільки він|
...
> **Примітка**:
>
> Незважаючи на те, що `=` має нижчий пріоритет, ніж більшість
> інших операторів, PHP все ж таки дозволяє робити так:
> `if (!$a = foo())`, у цьому прикладі результат виконання `foo()` буде
> присвоєно `$a`.
