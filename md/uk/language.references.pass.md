...
` <?phpfunction foo(&$var) {    $var++;}$a = 5;foo($a);// $a тут рівно 6?> `
...
` <?phpfunction foo(&$var) {   $var++;}function bar() { // Операція & відсутня    $a = 5; return$a;}foo(bar()); // Викликає попередження foo ($a = 5); // Вираження, а не зміннаfoo(5); // Константа, а не зміннаclass Foobar{}foo(new Foobar()) // Викликає повідомлення с PHP 7.0.7       
