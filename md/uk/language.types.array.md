...
#### Визначення за допомогою [array()](function.array.html)

Масив (тип array) може бути створений мовною конструкцією [array()](function.array.html). Як параметри вона приймає будь-яку кількість розділених комами пар `key => value` (Ключ => значення).
...
Для зміни певного значення просто надайте нове значення елементу, використовуючи його ключ. Якщо ви бажаєте видалити пару ключ/значення, вам необхідно використовувати функцію [unset()](function.unset.html)
...
Масиви можуть бути деструктуровані за допомогою мовних конструкцій `[]` (починаючи з PHP 7.1.0) або [list()](function.list.html). Ці конструкції можуть бути використані для деструктуризації масиву окремі змінні.
...
Для роботи з масивами існує достатньо корисних функцій. Дивіться розділ [функции для работы с массивами](ref.array.html)

> **Зауваження**
> 
> Функція [unset()](function.unset.html) дозволяє видаляти ключі масиву. Зверніть увагу, що масив *НЕ* буде переіндексовано. Якщо ви дійсно хочете поведінки в стилі "видалити і зрушити", можна переіндексувати масив використовуючи [array\_values()](function.array-values.html)
> 
> ```php
> <?php
...
Резервний варіант для обробки невизначеної константи як порожнього рядка викликає помилку рівня **`E_NOTICE`**. Починаючи з PHP 7.2.0, поведінка оголошена застарілою і викликає помилку рівня. **`E_WARNING`**. Починаючи з PHP 8.0.0, видалено та викидає виняток [Error](class.error.html)

> **Зауваження**: Це не означає, що потрібно *завжди* укладати ключ у лапки. Немає необхідності укладати в лапки [константы](language.constants.html) або [переменные](language.variables.html)оскільки це завадить PHP обробляти їх.
> 
> ```php
> <?php
...
Якщо ви переведете [error\_reporting](errorfunc.configuration.html#ini.error-reporting) режим відображення помилок рівня **`E_NOTICE`** (наприклад, такий як **`E_ALL`**), Ви відразу побачите ці помилки. За замовчуванням [error\_reporting](errorfunc.configuration.html#ini.error-reporting) встановлена ​​їх не відображати.
...
Колись у майбутньому команда розробників PHP можливо захоче додати ще одну константу або ключове слово, або константа з іншого коду може втрутитися і тоді у вас можуть виникнути проблеми. Наприклад, ви вже не можете використовувати таким чином слова `empty` і `default`оскільки вони є [зарезервированными ключевыми словами](reserved.html)

> **Зауваження**: Повторимо, всередині рядка (string), укладеного в подвійні лапки, коректно не оточувати індекси масиву лапками, тому `"$foo[bar]"` є вірним записом. Більш докладно чому - дивіться наведені вище приклади, а також розділ по [обработке переменных в строках](language.types.string.html#language.types.string.parsing)
...
Масиви можна порівнювати за допомогою функції [array\_diff()](function.array-diff.html) і [операторов массивов](language.operators.array.html)

### Розпакування масиву

Масив із префіксом `...` буде розпакований під час визначення масиву. Тільки масиви та об'єкти, які реалізують інтерфейс [Traversable](class.traversable.html), можуть бути розпаковані. Розпакування масиву за допомогою `...` доступна починаючи з PHP 7.4.0.
...
Розпакування масиву за допомогою оператора `...` слідує семантиці функції [array\_merge()](function.array-merge.html). Тобто пізніші рядкові ключі перезаписують раніші, а цілочисленні ключі перенумеровуються:
...
> **Зауваження**
> 
> Ключі, які не є ні цілими числами, ні рядками, викликають помилку [TypeError](class.typeerror.html). Такі ключі можуть бути згенеровані лише об'єктом [Traversable](class.traversable.html)
...
Масиви впорядковані. Ви можете змінювати порядок елементів за допомогою різних функцій сортування. Для додаткової інформації дивіться розділ [функции для работы с массивами](ref.array.html). Ви можете підрахувати кількість елементів у масиві за допомогою функції [count()](function.count.html)
...
Зауважте, що при присвоєнні масиву завжди відбувається копіювання значення. Щоб скопіювати масив за посиланням, потрібно використовувати [оператор ссылки](language.operators.html)
