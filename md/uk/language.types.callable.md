...
` <?php// Наше замикання$double = function($a) {    return $a * 2;};// Діапазон чисел$numbers = range(1, 5);// Ін для подвоєння кожного елемента в нашому діапазоні$new_numbers = array_map($double, $numbers);print implode(' ', $new_numbers);?> `
