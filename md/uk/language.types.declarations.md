...
[mixed](language.types.declarations.html#language.types.declarations.mixed) еквівалентний [объединению типов](language.types.declarations.html#language.types.declarations.composite.union) object|resource|array|string|int|float|bool|null. Доступно з PHP 8.0.0.
...
Псевдотип `false` підтримується як частина об'єднання типів. Він доданий з історичних причин, оскільки багато вбудованих функцій повертають `false` замість `null` у разі виникнення помилки. Класичний приклад – функція [strpos()](function.strpos.html)
...
-   Використання типу [mixed](language.types.declarations.html#language.types.declarations.mixed) призводить до помилки.
-   Для об'єднання типів:
    ...
    -   Якщо використовується [iterable](language.types.iterable.html), то до нього не можна додати array або [Traversable](class.traversable.html)
-   Для перетину типів:
...
`never` - тип значення, що повертається, що вказує, що функція нічого не повертає. Це означає, що вона або викликає [exit()](function.exit.html), або викидає виняток, або є нескінченним циклом. Отже, вона не може бути частиною оголошення union-типу. Доступно з PHP 8.1.0.
...
Значення має бути [`instanceof`](language.operators.type.html) того ж класу, в якому було викликано метод. Доступно з PHP 8.0.0.
...
Можна увімкнути режим суворої типізації на рівні файлу. У цьому режимі тип значення повинен суворо відповідати оголошеному, інакше буде викинуто виняток [TypeError](class.typeerror.html). Єдиним винятком цього правила є передача значення типу int туди, де очікується float.

**Увага**

На дзвінки з внутрішніх функцій, дія `strict_types` не розповсюджується.

Для увімкнення суворої типізації використовується оператор [`declare`](control-structures.declare.html) з оголошенням `strict_types`
...
**Приклад #13 [TypeError](class.typeerror.html)**
