...
Так що ніколи не довіряйте точності чисел з плаваючою точкою до останньої цифри і не перевіряйте прямо їхню рівність. Якщо вам дійсно потрібна висока точність, використовуйте [математичні функції довільної точності](ref.bc.html) і [gmp](ref.gmp.html)функції.
...
Якщо рядок [містить число](language.types.numeric-strings.html) або провідну числову послідовність, тоді вона буде перетворена у відповідне значення з плаваючою точкою, в іншому випадку вона перетворюється на нуль (`0`
...
Так як **`NAN`** є необмежену кількість різних значень, то **`NAN`** не слід порівнювати з іншими значеннями, включаючи її саму. Натомість, для визначення її наявності необхідно використовувати функцію [ісnan()](function.is-nan.html)