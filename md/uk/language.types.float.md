...
Для значень інших типів перетворення виконується шляхом перетворення значення спочатку ціле число (int), а потім число з плаваючою точкою ( float ). Дивіться [Преобразование в целое число](language.types.integer.md#language.types.integer.casting) для отримання додаткової інформації.
