...
` <?php$a = 1.234;$b = 1.2e3;$c = 7E-10;$d = 1_234.567; // починаючи с PHP 7.4.0?> `
...
дійсно необхідна висока точність, використовуйте [математичні функції довільної точності](ref.bc.md) та
[gmp](ref.gmp.md)-функції.

"Просте" пояснення можна знайти в [» посібнику за числами з плаваючою точкою](http://floating-point-gui.de/), яке також називається "Why
...
плаваючою точкою (float). Дивіться [Перетворення на ціле число](language.types.integer.md#language.types.integer.casting) для
...
`<?php$a = 1.23456789;$b = 1.23456780;$epsilon = 0.00001;if (abs($a - $b) < $epsilon) {     
