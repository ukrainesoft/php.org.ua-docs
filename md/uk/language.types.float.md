...
Так що ніколи не довіряйте точності чисел з плаваючою точкою до останньої цифри і не перевіряйте прямо їхню рівність. Якщо вам дійсно потрібна висока точність, використовуйте [математические функции произвольной точности](ref.bc.html) і [gmp](ref.gmp.html)функції.

"Просте" пояснення можна знайти в [» руководстве по числам с плавающей точкой](http://floating-point-gui.de/), яка також називається "Why don’t my numbers add up?" ("Чому мої числа не складаються?")

### Перетворення на число з плаваючою точкою

#### З рядків

Якщо рядок [содержит число](language.types.numeric-strings.html) або провідну числову послідовність, тоді вона буде перетворена у відповідне значення з плаваючою точкою, в іншому випадку вона перетворюється на нуль (`0`
...
Так як **`NAN`** є необмежену кількість різних значень, то **`NAN`** не слід порівнювати з іншими значеннями, включаючи її саму. Натомість, для визначення її наявності необхідно використовувати функцію [is\_nan()](function.is-nan.html)