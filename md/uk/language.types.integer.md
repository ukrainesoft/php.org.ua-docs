...
цілих (int) використовується [оператор заперечення](language.operators.arithmetic.md)
...
`<?php$a = 1234; // десяткове число$a = 0123; // восьмеричне число (еквівалентно 83 в десятковій системі)$a = 0o123; // восьмеричне число (починаючи з PHP 8.1.0)$a = 0x1A; // шістнадцяткове число (еквівалентно 26 в десятковій системі)$a = 0b11111111; // двійкове число (еквівалентно 255 в десятковій системі)$a = 1_234_567; // десяткове число(с PHP 7.4.0)?> `
...
`+/- 2.15e+9 = 2^31` на 32-бітних системах та `+/- 9.22e+18 = 2^63` на
...
Докладніше: [попередження про точність чисел з плаваючою точкою](language.types.float.md#warn.float-precision).
