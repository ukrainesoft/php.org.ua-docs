...
> **Зауваження**: На відміну від синтаксису. [подвійних лапок](language.types.string.html#language.types.string.syntax.double) і [heredoc](language.types.string.html#language.types.string.syntax.heredoc) [переменные](language.variables.html) та керуючі послідовності для спеціальних символів, укладених у одинарні лапки, *не* обробляються.
...
Рядки можуть бути об'єднані за допомогою оператора '.' (крапка). Зверніть увагу, оператор додавання '+' *тут не працює*. Додаткову інформацію дивіться у розділі [Рядкові оператори](language.operators.string.html)

Для модифікації рядків існує багато корисних функцій.

Основні функції описані у розділі [рядкових функцій](ref.strings.html), а для розширеного пошуку та заміни - функції [Perl-сумісних регулярних виразів](ref.pcre.html)

Також існують [функції для роботи з URL](ref.url.html), та функції шифрування/дешифрування рядків ([Sodium](ref.sodium.html) і [Hash](ref.hash.html)

Зрештою, дивіться також [функції символьних типів](ref.ctype.html)

### Перетворення на рядок

Значення може бути перетворене на рядок за допомогою приведення `(string)`, або функції [strval()](function.strval.html). У виразах, де потрібний рядок, перетворення відбувається автоматично. Це відбувається, коли ви використовуєте функції [echo](function.echo.html) або [print](function.print.html)або коли значення змінної порівнюється з рядком. Прочитання розділів керівництва [Типы](language.types.html) і [Маніпуляції з типами](language.types.type-juggling.html) зробить наступне зрозумілішим. Дивіться також [settype()](function.settype.html)
...
Для перетворення змінної типу `"Object"` у тип string використовується магічний метод [toString](language.oop5.magic.html)
