...
> **Зауваження** `$this` - це спеціальна змінна, якій не можна нічого надавати. До PHP 7.1.0 було можливе опосередковане присвоєння (наприклад, з використанням [змінних змінних](language.variables.variable.html)

**Підказка**

Дивіться також [Руководство по именованию](userlandnaming.html)

Для отримання інформації про функції роботи зі змінними звертайтесь до розділу [функцій роботи зі змінними](ref.var.html)
...
За умовчанням змінні завжди надаються за значенням. Тобто, коли ви надаєте вираз змінної, все значення оригінального виразу копіюється в цю змінну. Це означає, наприклад, що як однієї змінної присвоєно значення інший, зміна однієї з них впливає іншу. Додаткову інформацію про цей спосіб присвоєння дивіться у розділі [Вирази](language.expressions.html)

PHP також пропонує інший спосіб присвоєння значень змінним: [присвоєння за посиланням](language.references.html). Це означає, що нова змінна просто посилається (інакше кажучи, стає псевдонімом або вказує) на оригінальну змінну. Зміни у новій змінній відбиваються на оригіналі, і навпаки.

Для присвоєння за посиланням просто додайте амперсанд (&) до початку імені присвоюваної (вихідної) змінної. Наприклад, наступний фрагмент коду двічі виводить '`Меня зовут Боб`
...
Хорошою практикою вважається ініціалізувати змінні, хоча в PHP це не є обов'язковою вимогою. Неініціалізовані змінні набувають значення за умовчанням залежно від їх типу, що визначається з контексту їх першого використання: булеви набувають значення **`false`**, цілі числа та числа з плаваючою точкою - нуль, рядки (наприклад, при використанні в [echo](function.echo.html)) - порожній рядок, а масиви стають порожніми масивами.
...
// Использование в качестве Масива; выведет array(1) {  [3]=>  string(3) "def" }
