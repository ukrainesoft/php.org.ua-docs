...
> (наприклад, з використанням [змінних > змінних](language.variables.variable.md)).
...
` <?php$var = 'Боб';$Var = 'Джо';echo "$var, $Var"; // виведе "Боб, Джо"$4site = 'є ще ні'; // неправильно; починається з цифри$_4site = 'ще ні'; // Правильно; починається з символу підкреслення$täyte = 'mansikka'; // Правильно; 'ä' це (Розширений) ASCII 228.?> `
...
`<?php$foo = 'Боб'; // Привласнює $foo значення 'Боб'$bar = &$foo; // Посилання на $foo через $bar.$bar = "Мене звуть $bar"; // Зміна $bar...echo $bar;echo $foo; // мінює і $foo.?> `

За посиланням можуть бути присвоєні лише іменовані
змінні.

` <?php$foo = 25;$bar = &$foo; // Це вірне присвоєння.$bar = &(24 * 7); // Неправильно; посилання на неіменоване вираз.function test(){  return 25;}$bar = &test(); // Неправильно.?> `
...
" : "false
");// Строкове використання; виведе 'string(3) "abc"'$unset_str .= 'abc';var_dump($unset_str);// Цілочисленне використання; виведе 'int(25)'$unset_int 5 // 0 + 25 => 25var_dump($unset_int);// Використання в кількості  з плаваючою точкою(float/double); виведе 'float(1.25)'$1; як масиву; виведе array(1) {  [3]=> string(3) "def" }$unset_arr[3] = "def"; // array() + array(3 =>> > array(3 => "def")var_dump($unset_arr);// Використання якобъекта; створює новий об'єкт stdClass (дивіться http://www.php.net/manual/ru/reserved.classes. Виведе: object(stdClass)#1 (1) {  ["foo"]=>  string(3) "bar" }$unset_obj->foo = 'bar';var_dump($unset_obj);?> `
