...
>
> ` <?php// $ds - коректний ідентифікатор з'єднання з сервером каталогів$result = ldap_read($ds, '', '(objectClass=*)', ['supportedControl']);if (!in_array(LDAP) ($ds, $result)[0]['supportedcontrol'])) { die("Цей сервер не підтримує управління пагінацією");}?> `
