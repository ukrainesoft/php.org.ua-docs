...
Будь ласка, зверніть увагу: `port` за умовчанням дорівнює [memcache.defaultport](memcache.ini.md#ini.memcache.default-port). Тому рекомендується завжди вказувати номер порту під час використання цього методу.
