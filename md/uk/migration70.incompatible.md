...
масиву під час ітерації](migration70.incompatible.md#migration70.incompatible.foreach.by-ref)
...
` <?php$str = "0xffff";$int = filter_var($str, FILTER_VALIDATE_INT, FILTER_FLAG_ALLOW_HEX);if (false === $int) {                  ¦¦¦ int); // int (65535)?

#### `\u{` може викликати помилки

У зв'язку з додаванням нового [синтаксису екранування кодів Unicode](migration70.new-features.md#migration70.new-features.unicode-codepoint-escape-syntax),
...
вам також буде цікаво розглянути можливість [звернення до функцій через змінні](functions.variable-functions.md) та/або оператор
...
іншого MySQL API дивіться розділ [Вибір MySQL API](mysqlinfo.api.choosing.md).
...
[Раніше визнані застарілими у PHP 5.6](migration56.deprecated.md#migration56.deprecated.incompatible-context)
...
оператором з пріоритетом між `print` та `=>`. Це може призвести до
зміни поведінки:
