...
Багато фатальних і поправних фатальних помилок було перероблено у винятки в PHP 7. Ці винятки успадковують клас [Error](class.error.html), який, у свою чергу, реалізує інтерфейс [Throwable](class.throwable.html) (Новий базовий інтерфейс, який успадковують усі винятки).

Це означає, що користувальницькі обробники помилок можуть не бути викликані, тому що замість виклику помилки, буде викинуто виняток (породжуючи нові фатальні помилки через неперехоплені винятки класу [Error](class.error.html)
...
Раніше деякі внутрішні класи повертали **`null`** або марний об'єкт, коли конструктор зазнав невдачі. Усі вбудовані класи тепер у такому разі викидатимуть виняток [Exception](class.exception.html), Як це вже роблять користувальницькі класи.

#### Помилки розбору кидають виняток класу [ParseError](class.parseerror.html)

Помилки розбору кидають виняток класу [ParseError](class.parseerror.html). Обробка помилок [eval()](function.eval.html) повинна включати блок [`catch`](language.exceptions.html#language.exceptions.catch), який ловитиме цю помилку.
...
#### Зміна в обробці [list()](function.list.html)

##### [list()](function.list.html) більше не надає змінних у зворотному порядку

Тепер [list()](function.list.html) привласнює змінні тому порядку, як вони перераховані, а чи не у протилежному. Загалом це впливає тільки на випадки, коли [list()](function.list.html) використовується спільно з оператором масиву `[]`, як показано нижче:
...
Також хочеться відзначити, що покладатися на порядок присвоєння оператором [list()](function.list.html) - не найрозумніше рішення, оскільки він знову може змінитися у майбутньому.

##### Порожнє присвоєння [list()](function.list.html) більше не дозволено

Конструкція [list()](function.list.html) більше не може бути порожнім. Наступні приклади неприпустимі:
...
#### Ітерація об'єктів, що не реалізують [Traversable](class.traversable.html)

Ітерація об'єктів, що не реалізують [Traversable](class.traversable.html) тепер відбувається так само, як і ітерація масиву за посиланням. Так виходить через те, що [улучшения поведения при изменении массива во время итерации](migration70.incompatible.html#migration70.incompatible.foreach.by-ref) також впливає при додаванні або видаленні властивостей об'єкта.
...
Тепер побитові зміщення на негативну величину кидатимуть виняток [ArithmeticError](class.arithmeticerror.html)
...
У зв'язку з додаванням нового [синтаксиса экранирования кодов Unicode](migration70.new-features.html#migration70.new-features.unicode-codepoint-escape-syntax), рядки, що містять рядок `\u{`, що попереджає некоректну послідовність, може призвести до фатальної помилки. Для того, щоб цього уникнути, необхідно екранувати перший зворотний сліш.
...
Усі функції `ereg` видалено. Рекомендована альтернатива - [PCRE](book.pcre.html)
...
Усі функції [ext/mysql](book.mysql.html) були вилучені. Для вибору іншого MySQL API дивіться розділ [Выбор MySQL API](mysqlinfo.api.choosing.html)
...
Директива `xsl.security_prefs` було видалено. Замість неї контролю налаштувань безпеки повинен викликатися метод [XsltProcessor::setSecurityPrefs()](xsltprocessor.setsecurityprefs.html) лише на рівні кожного процесора.
...
[Ранее признанные устаревшими в PHP 5.6](migration56.deprecated.html#migration56.deprecated.incompatible-context) статичні виклики нестатичних методів з невідповідного контексту тепер призведуть до того, що для методу, що викликається, змінна `$this` буде не визначено та буде виведено попередження.
