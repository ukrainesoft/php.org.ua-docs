...
У PHP 7 додано підтримку [объявления возвращаемого типа](language.types.declarations.html). Аналогічно як і [объявления типов аргументов](language.types.declarations.html), оголошення типу значення, що повертається вказує, значення якого типу повинна повернути функція. Для оголошення типу значення, що повертається доступні все ті ж [типы](language.types.declarations.html)що для оголошення типів аргументів.
...
Повну документацію та приклади використання читайте у розділі про [объявление возвращаемого типа](language.types.declarations.html)
...
Цей оператор призначений для порівняння двох виразів. Він повертає -1, 0 або 1, якщо $a, відповідно, менше, дорівнює або більше ніж $b. Порівняння проводиться відповідно до [правилами сравнения типов](types.comparisons.html) PHP.
...
Детальну документацію та приклади використання читайте у розділі про [анонимные классы](language.oop5.anonymous.html)
...
Тепер [session\_start()](function.session-start.html) приймає масив опцій, які перевизначать [конфигурационные директивы сессии](session.configuration.html) встановлені у php.ini.

Також опції були розширені включеною за замовчуванням опцією [session.lazy\_write](session.configuration.html#ini.session.lazy-write), яка говорить PHP про те, що файл сесії треба перезаписувати тільки якщо змінилися дані сесії, і опцією `read_and_close`, яку можна задати тільки через [session\_start()](function.session-start.html) для того, щоб PHP закривав сесію відразу ж як прочитає її дані і не вносив до неї будь-яких змін.

Наприклад, для встановлення [session.cache\_limiter](session.configuration.html#ini.session.cache-limiter) рівним `private` та негайного закриття сесії після читання її даних:
...
Нова функція [preg\_replace\_callback\_array()](function.preg-replace-callback-array.html) дозволяє писати чистіший код, коли потрібно використовувати функцію [preg\_replace\_callback()](function.preg-replace-callback.html). До PHP 7 при необхідності обробити різні регулярні вирази різними функціями доводилося для кожної такої обробки писати окремий виклик функції.
...
Було додано дві нові кросплатформні функції для генерації криптографічно безпечних рядків і цілих чисел: [random\_bytes()](function.random-bytes.html) і [random\_int()](function.random-int.html)
