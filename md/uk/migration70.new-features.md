...
У PHP 7 додано підтримку [оголошення типу, що повертається](language.types.declarations.html). Аналогічно як і [объявления типов аргументов](language.types.declarations.html), оголошення типу значення, що повертається вказує, значення якого типу повинна повернути функція. Для оголошення типу значення, що повертається доступні все ті ж [типы](language.types.declarations.html)що для оголошення типів аргументів.
