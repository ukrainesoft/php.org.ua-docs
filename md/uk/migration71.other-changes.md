...
navigation:
  ...
  - migration71.windows-support.md: Підтримка Windows »
  - index.md: PHP Manual
...
-   `session.sid_length`\- Визначає довжину ідентифікатора сесії. За промовчанням використовується 32 символи для забезпечення зворотної сумісності.
...
Теперь исключение[TypeError](class.typeerror.md) для перевірок типу arg\_info надає більш інформативне повідомлення про помилку. Якщо тип параметра або значення, що повертається, може бути **`null`**(либо имеет значение\*\*`null`\*\* за замовчуванням або має тип, що обнулюється), то тепер у повідомленні буде згадано "must be ... or null" або "must ... or be null."
