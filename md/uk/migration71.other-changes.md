...
### Більш інформативне повідомлення [TypeError](class.typeerror.html), якщо допустимо **`null`**

Тепер виняток [TypeError](class.typeerror.html) для перевірок типу arginfo надає більш інформативне повідомлення про помилку. Якщо тип параметра або значення, що повертається, може бути **`null`** (або має значення **`null`** за замовчуванням або має тип, що обнулюється), то тепер у повідомленні буде згадано "must be ... or null" або "must ... or be null."