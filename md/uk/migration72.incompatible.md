...
Тепер числові ключі краще обробляються при приведенні масивів до об'єктів та об'єктів до масивів (через явне приведення, або використовуючи [settype()](function.settype.html)
...
Тепер при використанні [count()](function.count.html) з параметром, який не можна вважати буде виникати помилка рівня **`E_WARNING`** (це також стосується [sizeof()](function.sizeof.html) як псевдоніма цієї функції).
...
### Значення, що повертається [gettype()](function.gettype.html) для закритих ресурсів
...
Не повністю визначені посилання на невизначені константи тепер генеруватимуть **`E_WARNING`** (замість **`E_NOTICE`**). У наступній основній версії PHP вони генеруватимуть винятки [Error](class.error.html)
...
### Зміни [bcmod()](function.bcmod.html) з числами з плаваючою точкою

Функція [bcmod()](function.bcmod.html) більше не обрізає числа з плаваючою точкою до цілих. Таким чином, її поведінка тепер відповідає [fmod()](function.fmod.html), а не оператору `%`. Наприклад, `bcmod('4', '3.5')` тепер повертає `0.5` замість `1`
