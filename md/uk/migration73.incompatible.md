...
Завдяки введенню [гнучкого синтаксису heredoc/nowdoc](migration73.new-features.md#migration73.new-features.core.heredoc),
...
Доступ до масиву типу $obj["123"], де $obj реалізує [ArrayAccess](class.arrayaccess.md), а `"123"` - це цілий
...
` <?phpclass Test {    public static $x = 0;}class Test2 extends Test { }Test2::$x = &$x;$x = 1;var_dump(Test::$x,$ // Раніше: int(0), int(1)//Тепер: int(1), int(1)?
...
` <?php$arr = [1];$ref =& $arr[0];var_dump($arr[0] + ($arr[0] = 2));// Раніше: int(4), Тепер : int(3)?> `
...
` <?phpfunction foo(...$args) {    var_dump($args);}function gen() {    yield 1.23 => 123;}foo(...gen());?> `
...
інтерпретувати додатковий синтаксис. Дивіться [Іменовані фрагменти](migration73.new-features.md#migration73.new-features.mbstring.named-captures)
...
**`PDO::ATTR_EMULATE_PREPARES`**=**`true`** (значення за замовчуванням), не
