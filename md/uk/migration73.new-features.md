...
-   Підтримка **`LDAP_OPT_SERVER_CONTROLS`** і **`LDAP_OPT_CLIENT_CONTROLS`** у функціях [ldapgetoption()](function.ldap-get-option.html) і [ldapsetoption()](function.ldap-set-option.html) було виправлено.
...
Різні режими перетворення та порівняння регістру символів доступні у функції [мбconvertcase()](function.mb-convert-case.html)

-   **`MB_CASE_LOWER`** (використовується [мбstrtolower()](function.mb-strtolower.html)
-   **`MB_CASE_UPPER`** (використовується [мбstrtoupper()](function.mb-strtoupper.html)
-   **`MB_CASE_TITLE`**
-   **`MB_CASE_FOLD`**
...
Функція [мбconvertcase()](function.mb-convert-case.html) з **`MB_CASE_TITLE`** тепер виконує перетворення title-case в залежності від властивостей Unicode, заснованих на Cased та CaseIgnorable. Зокрема, це також покращує обробку лапок та апострофів.
...
Функції `mb_ereg_*` Тепер підтримують іменовані фрагменти. Відповідні функції, такі як [мбereg()](function.mb-ereg.html), тепер повертатимуть іменовані фрагменти як зі своїм номером групи, і зі своїм ім'ям, аналогічно PCRE:
...
Крім того, функція [мбeregreplace()](function.mb-ereg-replace.html) тепер підтримує позначення `\k<>` і `\k''` для посилання на іменовані фрагменти в рядку, що замінює:
...
У функції [readlineinfo()](function.readline-info.html) додано підтримку параметрів `completion_append_character` і `completion_suppress_append`. Ці опції доступні, тільки якщо PHP скомпільовано з бібліотекою libreadline (а не libedit).