...
за допомогою синтаксису `[&$a, [$b, &$c]] = $d`. Те саме підтримується і
...
З аргументом **--with-password-argon2[=dir]** скрипта configure тепер
...
` <?phpmb_ereg('(?<word>\w+)', '국', $matches);// => [0 => "국", 1 => "국", "word" => "국 "];?> `
