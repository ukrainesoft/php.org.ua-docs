---
navigation:
  - migration81.other-changes.md: « Інші зміни
  - migration80.new-features.md: Нова функціональність »
  - index.md: PHP Manual
  - appendices.md: Програми
title: Міграція з PHP 7.4.x на PHP 8.0.x
origin_hash: ddf652f5224dc9f1fa9671347921941ca401ea50
---
# Міграція з PHP 7.4.x на PHP 8.0.x

## Зміст

-   [Нова функціональність](migration80.new-features.md)
-   [Зміни, що ламають зворотну сумісність](migration80.incompatible.md)
-   [Функціональність, оголошена застарілою в PHP 8.0.x](migration80.deprecated.md)
-   [Інші зміни](migration80.other-changes.md)

Нова версія містить ряд [нових функцій](migration80.new-features.md) і [несумісностей](migration80.incompatible.md), наявність яких слід перевірити перед перемиканням версій PHP у виробничому середовищі.

Смотрите также руководство по миграции для версий PHP[7.0.x](migration70.md) [7.1.x](migration71.md) [7.2.x](migration72.md) [7.3.x](migration73.md) і [7.4.x](migration74.md)
