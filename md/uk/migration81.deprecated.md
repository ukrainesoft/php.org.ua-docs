...
` <?php$a = [];$a[15.5]; // застаріло, оскільки значення ключа втрачає компонент 0.5$a[15.0]; // працює, так як 15.0 == 15?> `
...
> дозволено:
>
> ` <?php// З невизначеного значення$arr[] = 'якесь значення';$arr['doesNotExist'][] = 2;// З null$arr = null;$arr[] = ?> `
