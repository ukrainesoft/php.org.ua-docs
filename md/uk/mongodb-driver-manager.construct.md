...
Докладніше про параметри, що підтримуються URI, дивіться розділ [» Параметры строки подключения](https://www.mongodb.com/docs/manual/reference/connection-string/#connections-connection-options) у посібнику MongoDB . [» Параметры пула подключения](https://www.mongodb.com/docs/manual/reference/connection-string/#connection-pool-options) не підтримуються, т.к. PHP-драйвер не продає пули підключень.

`uri` - це URL, тому будь-які спеціальні символи в його компонентах повинні бути закодовані відповідно [» RFC 3986](http://www.faqs.org/rfcs/rfc3986). Це особливо актуально для імені користувача та пароля, які часто можуть містити спеціальні символи, такі як `@` `:`, або `%`. При підключенні через доменний сокет Unix шлях сокету може містити спеціальні символи, наприклад сліші, які необхідно закодувати. Функція [rawurlencode()](function.rawurlencode.html) може використовуватися для кодування складових частин URI-адреси.
...
Додаткові [» параметры строки подключения](https://www.mongodb.com/docs/manual/reference/connection-string/#connections-connection-options), які будуть перезаписувати будь-які параметри з тим самим ім'ям у параметрі `uri`
...
Механізм аутентифікації, який MongoDB використовуватиме для аутентифікації з'єднання. Щоб переглянути додаткові відомості та список підтримуваних значень, див. [» Параметры аутентификации](https://www.mongodb.com/docs/manual/reference/connection-string/#urioption.authMechanism) у посібнику MongoDB.
...
Підключення до автономного сайту MongoDB через доменний сокет Unix. Шлях сокету може містити спеціальні символи, наприклад сліші, які мають закодувати за допомогою [rawurlencode()](function.rawurlencode.html)
