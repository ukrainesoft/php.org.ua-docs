...
Перевіряє, чи ця гарантія запису за промовчанням (тобто не вказані параметри). Цей метод в першу чергу призначений для використання спільно з [MongoDBDriverManager::getWriteConcern()](mongodb-driver-manager.getwriteconcern.html) для визначення того, чи об'єкт Manager був створений без будь-яких опцій гарантії запису.
