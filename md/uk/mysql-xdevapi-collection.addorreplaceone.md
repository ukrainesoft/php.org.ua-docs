...
` <?php$session = mysql_xdevapi\getSession("mysqlx://user:password@localhost");$session->sql("DROP DATABASE IF EXISTS addressbook")->execute();$session->sql( "CREATE DATABASE addressbook")->execute();$schema = $session->getSchema("addressbook");$create = $schema->createCollection("people");$collection = $schema->get people");// Використання add()$result = $collection->add('{"name": "Wilma", "age": 23, "job": "Teacher"}')->execute() ;// Використання addOrReplaceOne()// Примітки: ми передаємо відоме значення _id$result = $collection->addOrReplaceOne('00005b6b5361000000000000       Construction"}');?> `
