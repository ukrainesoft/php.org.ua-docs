...
Якщо перед ім'ям хоста встановити рядок `p:`, то буде відкрито постійне з'єднання. Якщо з'єднання відкрито з пулу підключень, буде автоматично викликано функцію [mysqli\_change\_user()](mysqli.change-user.html)
...
[mysqli\_connect()](function.mysqli-connect.html) повертає об'єкт, який представляє з'єднання з сервером MySQL або **`false`** у разі виникнення помилки.

[mysqli::connect()](function.mysqli-connect.html) повертає **`null`** у разі успішного виконання або **`false`** у разі виникнення помилки.

### Помилки

Якщо **`MYSQLI_REPORT_STRICT`** увімкнена та під час підключення до бази даних сталася помилка, викидається [mysqli\_sql\_exception](class.mysqli-sql-exception.html)
...
> 
> За замовчуванням Libmysqlclient використовує кодування, встановлене в my.cnf або спеціальним викликом [mysqli\_options()](mysqli.options.html) до використання [mysqli\_real\_connect()](mysqli.real-connect.html), але після [mysqli\_init()](mysqli.init.html)

> **Зауваження**
> 
> Якщо використовується Об'єктно-орієнтований стиль: Якщо з'єднання встановити не вдалося, метод все одно поверне об'єкт. Перевірити успішність створення підключення можна або функцією [mysqli\_connect\_error()](mysqli.connect-error.html) або за допомогою властивості [mysqli->connect\_error](mysqli.connect-error.html), Як показано в прикладах.

> **Зауваження**
> 
> Якщо необхідно задати додаткові параметри підключення, на кшталт часу очікування і т.п., замість цього методу необхідно використовувати функцію [mysqli\_real\_connect()](mysqli.real-connect.html)

> **Зауваження**
> 
> Виклик конструктора без параметрів ідентичний виклику функції [mysqli\_init()](mysqli.init.html)

> **Зауваження**
> 
> Помилка "Can't create TCP/IP socket (10106)" зазвичай означає, що директива конфігурації [variables\_order](ini.core.html#ini.variables-order) не містить символ `E`. У Windows системах, якщо оточення не скопійовано, змінне середовище `SYSTEMROOT` буде недоступна, і у PHP виникнуть проблеми із завантаженням Winsock.
