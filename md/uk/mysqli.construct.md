...
Якщо перед ім'ям хоста встановити рядок `p:`, то буде відкрито постійне з'єднання. Якщо з'єднання відкрито з пулу підключень, буде автоматично викликано функцію [mysqlichangeuser()](mysqli.change-user.html)
...
[mysqliconnect()](function.mysqli-connect.html) повертає об'єкт, який представляє з'єднання з сервером MySQL або **`false`** у разі виникнення помилки.

[mysqli::connect()](function.mysqli-connect.html) повертає **`null`** у разі успішного виконання або **`false`** у разі виникнення помилки.

### Помилки

Якщо **`MYSQLI_REPORT_STRICT`** увімкнена та під час підключення до бази даних сталася помилка, викидається [mysqlisqlexception](class.mysqli-sql-exception.html)
...
> 
> За замовчуванням Libmysqlclient використовує кодування, встановлене в my.cnf або спеціальним викликом [mysqlioptions()](mysqli.options.html) до використання [mysqlirealconnect()](mysqli.real-connect.html), але після [mysqliinit()](mysqli.init.html)

> **Зауваження**
> 
> Якщо використовується Об'єктно-орієнтований стиль: Якщо з'єднання встановити не вдалося, метод все одно поверне об'єкт. Перевірити успішність створення підключення можна або функцією [mysqliconnecterror()](mysqli.connect-error.html) або за допомогою властивості [mysqli->connecterror](mysqli.connect-error.html), Як показано в прикладах.

> **Зауваження**
> 
> Якщо необхідно задати додаткові параметри підключення, на кшталт часу очікування і т.п., замість цього методу необхідно використовувати функцію [mysqlirealconnect()](mysqli.real-connect.html)

> **Зауваження**
> 
> Виклик конструктора без параметрів ідентичний виклику функції [mysqliinit()](mysqli.init.html)

> **Зауваження**
> 
> Помилка "Can't create TCP/IP socket (10106)" зазвичай означає, що директива конфігурації [variablesorder](ini.core.html#ini.variables-order) не містить символ `E`. У Windows системах, якщо оточення не скопійовано, змінне середовище `SYSTEMROOT` буде недоступна, і у PHP виникнуть проблеми із завантаженням Winsock.
