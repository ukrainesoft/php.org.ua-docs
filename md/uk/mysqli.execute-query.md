...
Підготовлений запит створюється під капотом, але він ніколи не виводиться за межі функції. Неможливо отримати доступ до властивостей запиту, як це можна зробити з об'єктом [mysqli\_stmt](class.mysqli-stmt.md). Через це обмеження інформація про стан копіюється в об'єкт [mysqli](class.mysqli.md)и доступна с помощью его методов, например,[mysqli\_affected\_rows()](mysqli.affected-rows.md) або [mysqli\_error()](mysqli.error.md)
...
**Пример #1 Пример использования**mysqli::execute\_query()\*\*\*\*
