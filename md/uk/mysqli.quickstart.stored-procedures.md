...
Результуючі таблиці процедур, що зберігаються, витягуються функціями [mysqli::realquery()](mysqli.real-query.html) або [mysqli::multiquery()](mysqli.multi-query.md). Обидві функції дозволяють отримати будь-яку кількість результуючих наборів, повернутих SQL-запитами, таких як `CALL`. Якщо в процесі роботи не вдається отримати всі доступні результати виклику процедури, що зберігається, буде викликатися помилка.
