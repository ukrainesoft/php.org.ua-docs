Основна інформація про функції модуля MySQLi

-   [« Обумовлені константи](mysqli.constants.html)
    
-   [mysqli »](class.mysqli.html)
    
-   [PHP Manual](index.html)
    
-   [MySQLi](book.mysqli.html)
    
-   Основна інформація про функції модуля MySQLi
    

# Основна інформація про функції модуля MySQLi

**Основна інформація про методи [mysqli](class.mysqli.html)**

| Класс mysqli                                                                                                              |
|---------------------------------------------------------------------------------------------------------------------------|
| ООП-інтерфейс                                                                                                             |
|                                                                                                                           |
| *Властивості*                                                                                                             |
| [$mysqli::affectedrows](mysqli.affected-rows.html)                                                                        |
| [$mysqli::clientinfo](mysqli.get-client-info.html)                                                                        |
| [$mysqli::clientversion](mysqli.get-client-version.html)                                                                  |
| [$mysqli::connecterrno](mysqli.connect-errno.html)                                                                        |
| [$mysqli::connecterror](mysqli.connect-error.html)                                                                        |
| [$mysqli::errno](mysqli.errno.html)                                                                                       |
| [$mysqli::error](mysqli.error.html)                                                                                       |
| [$mysqli::fieldcount](mysqli.field-count.html)                                                                            |
| [$mysqli::hostinfo](mysqli.get-host-info.html)                                                                            |
| [$mysqli::protocolversion](mysqli.get-proto-info.html)                                                                    |
| [$mysqli::serverinfo](mysqli.get-server-info.html)                                                                        |
| [$mysqli::serverversion](mysqli.get-server-version.html)                                                                  |
| [$mysqli::info](mysqli.info.html)                                                                                         |
| [$mysqli::insertід](mysqli.insert-id.html)                                                                                |
| [$mysqli::sqlstate](mysqli.sqlstate.html)                                                                                 |
| [$mysqli::warningcount](mysqli.warning-count.html)                                                                        |
| *Методи*                                                                                                                  |
| [mysqli::autocommit()](mysqli.autocommit.html)                                                                            |
| [mysqli::changeuser()](mysqli.change-user.html)                                                                           |
| [mysqli::charactersetname()](mysqli.character-set-name.html)                                                              |
| [mysqli::close()](mysqli.close.html)                                                                                      |
| [mysqli::commit()](mysqli.commit.html)                                                                                    |
| [mysqli::construct()](mysqli.construct.html)                                                                              |
| [mysqli::debug()](mysqli.debug.html)                                                                                      |
| [mysqli::dumpdebuginfo()](mysqli.dump-debug-info.html)                                                                    |
| [mysqli::getcharset()](mysqli.get-charset.html)                                                                           |
| [mysqli::getconnectionstats()](mysqli.get-connection-stats.html)                                                          |
| [mysqli::getclientinfo()](mysqli.get-client-info.html)                                                                    |
| Н/Д                                                                                                                       |
| [mysqli::getserverinfo()](mysqli.get-server-info.html)                                                                    |
| [mysqli::getwarnings()](mysqli.get-warnings.html)                                                                         |
| [mysqli::init()](mysqli.init.html)                                                                                        |
| [mysqli::kill()](mysqli.kill.html)                                                                                        |
| [mysqli::moreresults()](mysqli.more-results.html)                                                                         |
| [mysqli::multiquery()](mysqli.multi-query.html)                                                                           |
| [mysqli::nextresult()](mysqli.next-result.html)                                                                           |
| [mysqli::options()](mysqli.options.html)                                                                                  |
| [mysqli::ping()](mysqli.ping.html)                                                                                        |
| [mysqli::prepare()](mysqli.prepare.html)                                                                                  |
| [mysqli::query()](mysqli.query.html)                                                                                      |
| [mysqli::realconnect()](mysqli.real-connect.html)                                                                         |
| [mysqli::realescapestring()](mysqli.real-escape-string.html) [mysqli::escapestring()](function.mysqli-escape-string.html) |
| [mysqli::realquery()](mysqli.real-query.html)                                                                             |
| [mysqli::refresh()](mysqli.refresh.html)                                                                                  |
| [mysqli::rollback()](mysqli.rollback.html)                                                                                |
| [mysqli::selectdb()](mysqli.select-db.html)                                                                               |
| [mysqli::setcharset()](mysqli.set-charset.html)                                                                           |
| [mysqli::sslset()](mysqli.ssl-set.html)                                                                                   |
| [mysqli::stat()](mysqli.stat.html)                                                                                        |
| [mysqli::stmtinit()](mysqli.stmt-init.html)                                                                               |
| [mysqli::storeresult()](mysqli.store-result.html)                                                                         |
| [mysqli::threadid()](mysqli.thread-id.html)                                                                               |
| [mysqli::threadsafe()](mysqli.thread-safe.html)                                                                           |
| [mysqli::useresult()](mysqli.use-result.html)                                                                             |

**Основна інформація про методи [mysqlistmt](class.mysqli-stmt.html)**

| MySQL\_STMT                                                      |
|------------------------------------------------------------------|
| ООП-інтерфейс                                                    |
|                                                                  |
| *Властивості*                                                    |
| [$mysqlistmt::affectedrows](mysqli-stmt.affected-rows.html)      |
| [$mysqlistmt::errno](mysqli-stmt.errno.html)                     |
| [$mysqlistmt::error](mysqli-stmt.error.html)                     |
| [$mysqlistmt::fieldcount](mysqli-stmt.field-count.html)          |
| [$mysqlistmt::insertід](mysqli-stmt.insert-id.html)              |
| [$mysqlistmt::numrows](mysqli-stmt.num-rows.html)                |
| [$mysqlistmt::paramcount](mysqli-stmt.param-count.html)          |
| [$mysqlistmt::sqlstate](mysqli-stmt.sqlstate.html)               |
| *Методи*                                                         |
| [mysqlistmt::attrget()](mysqli-stmt.attr-get.html)               |
| [mysqlistmt::attrset()](mysqli-stmt.attr-set.html)               |
| [mysqlistmt::bindparam()](mysqli-stmt.bind-param.html)           |
| [mysqlistmt::bindresult()](mysqli-stmt.bind-result.html)         |
| [mysqlistmt::close()](mysqli-stmt.close.html)                    |
| [mysqlistmt::dataseek()](mysqli-stmt.data-seek.html)             |
| [mysqlistmt::execute()](mysqli-stmt.execute.html)                |
| [mysqlistmt::fetch()](mysqli-stmt.fetch.html)                    |
| [mysqlistmt::freeresult()](mysqli-stmt.free-result.html)         |
| [mysqlistmt::getresult()](mysqli-stmt.get-result.html)           |
| [mysqlistmt::getwarnings()](mysqli-stmt.get-warnings.html)       |
| [mysqlistmt::moreresults()](mysqli-stmt.more-results.html)       |
| [mysqlistmt::nextresult()](mysqli-stmt.next-result.html)         |
| [mysqlistmt::numrows()](mysqli-stmt.num-rows.html)               |
| [mysqlistmt::prepare()](mysqli-stmt.prepare.html)                |
| [mysqlistmt::reset()](mysqli-stmt.reset.html)                    |
| [mysqlistmt::resultmetadata()](mysqli-stmt.result-metadata.html) |
| [mysqlistmt::sendlongdata()](mysqli-stmt.send-long-data.html)    |
| [mysqlistmt::storeresult()](mysqli-stmt.store-result.html)       |

**Основна інформація про методи [mysqliresult](class.mysqli-result.html)**

| mysqli\_result                                                                                                                                     |
|----------------------------------------------------------------------------------------------------------------------------------------------------|
| ООП-інтерфейс                                                                                                                                      |
|                                                                                                                                                    |
| *Властивості*                                                                                                                                      |
| [$mysqliresult::currentfield](mysqli-result.current-field.html)                                                                                    |
| [$mysqliresult::fieldcount](mysqli-result.field-count.html)                                                                                        |
| [$mysqliresult::lengths](mysqli-result.lengths.html)                                                                                               |
| [$mysqliresult::numrows](mysqli-result.num-rows.html)                                                                                              |
| *Методи*                                                                                                                                           |
| [mysqliresult::dataseek()](mysqli-result.data-seek.html)                                                                                           |
| [mysqliresult::fetchall()](mysqli-result.fetch-all.html)                                                                                           |
| [mysqliresult::fetcharray()](mysqli-result.fetch-array.html)                                                                                       |
| [mysqliresult::fetchassoc()](mysqli-result.fetch-assoc.html)                                                                                       |
| [mysqliresult::fetchcolumn()](mysqli-result.fetch-column.html)                                                                                     |
| [mysqliresult::fetchfielddirect()](mysqli-result.fetch-field-direct.html)                                                                          |
| [mysqliresult::fetchfield()](mysqli-result.fetch-field.html)                                                                                       |
| [mysqliresult::fetchfields()](mysqli-result.fetch-fields.html)                                                                                     |
| [mysqliresult::fetchobject()](mysqli-result.fetch-object.html)                                                                                     |
| [mysqliresult::fetchrow()](mysqli-result.fetch-row.html)                                                                                           |
| [mysqliresult::fieldseek()](mysqli-result.field-seek.html)                                                                                         |
| [mysqliresult::free()](mysqli-result.free.html) [mysqliresult::close](mysqli-result.free.html) [mysqliresult::freeresult](mysqli-result.free.html) |

**Основна інформація про методи [mysqlidriver](class.mysqli-driver.html)**

| MySQL\_Driver                                                                   |
|---------------------------------------------------------------------------------|
| ООП-інтерфейс                                                                   |
|                                                                                 |
| *Властивості*                                                                   |
| [$mysqlidriver::mysqlireport](mysqli-driver.report-mode.html)                   |
| *Методи*                                                                        |
| [mysqlidriver::embeddedserverend()](mysqli-driver.embedded-server-end.html)     |
| [mysqlidriver::embeddedserverstart()](mysqli-driver.embedded-server-start.html) |

> **Зауваження**
> 
> Псевдоніми функцій призначені лише забезпечення зворотної сумісності. Не використовуйте їх у нових проектах.