Основна інформація про функції модуля MySQLi

-   [« Предопределённые константы](mysqli.constants.html)
    
-   [mysqli »](class.mysqli.html)
    
-   [PHP Manual](index.html)
    
-   [MySQLi](book.mysqli.html)
    
-   Основна інформація про функції модуля MySQLi
    

# Основна інформація про функції модуля MySQLi

**Основна інформація про методи [mysqli](class.mysqli.html)**

| Класс mysqli                                                                                                                    |
|---------------------------------------------------------------------------------------------------------------------------------|
| ООП-інтерфейс                                                                                                                   |
|                                                                                                                                 |
| *Властивості*                                                                                                                   |
| [$mysqli::affected\_rows](mysqli.affected-rows.html)                                                                            |
| [$mysqli::client\_info](mysqli.get-client-info.html)                                                                            |
| [$mysqli::client\_version](mysqli.get-client-version.html)                                                                      |
| [$mysqli::connect\_errno](mysqli.connect-errno.html)                                                                            |
| [$mysqli::connect\_error](mysqli.connect-error.html)                                                                            |
| [$mysqli::errno](mysqli.errno.html)                                                                                             |
| [$mysqli::error](mysqli.error.html)                                                                                             |
| [$mysqli::field\_count](mysqli.field-count.html)                                                                                |
| [$mysqli::host\_info](mysqli.get-host-info.html)                                                                                |
| [$mysqli::protocol\_version](mysqli.get-proto-info.html)                                                                        |
| [$mysqli::server\_info](mysqli.get-server-info.html)                                                                            |
| [$mysqli::server\_version](mysqli.get-server-version.html)                                                                      |
| [$mysqli::info](mysqli.info.html)                                                                                               |
| [$mysqli::insert\_id](mysqli.insert-id.html)                                                                                    |
| [$mysqli::sqlstate](mysqli.sqlstate.html)                                                                                       |
| [$mysqli::warning\_count](mysqli.warning-count.html)                                                                            |
| *Методи*                                                                                                                        |
| [mysqli::autocommit()](mysqli.autocommit.html)                                                                                  |
| [mysqli::change\_user()](mysqli.change-user.html)                                                                               |
| [mysqli::character\_set\_name()](mysqli.character-set-name.html)                                                                |
| [mysqli::close()](mysqli.close.html)                                                                                            |
| [mysqli::commit()](mysqli.commit.html)                                                                                          |
| [mysqli::\_\_construct()](mysqli.construct.html)                                                                                |
| [mysqli::debug()](mysqli.debug.html)                                                                                            |
| [mysqli::dump\_debug\_info()](mysqli.dump-debug-info.html)                                                                      |
| [mysqli::get\_charset()](mysqli.get-charset.html)                                                                               |
| [mysqli::get\_connection\_stats()](mysqli.get-connection-stats.html)                                                            |
| [mysqli::get\_client\_info()](mysqli.get-client-info.html)                                                                      |
| Н/Д                                                                                                                             |
| [mysqli::get\_server\_info()](mysqli.get-server-info.html)                                                                      |
| [mysqli::get\_warnings()](mysqli.get-warnings.html)                                                                             |
| [mysqli::init()](mysqli.init.html)                                                                                              |
| [mysqli::kill()](mysqli.kill.html)                                                                                              |
| [mysqli::more\_results()](mysqli.more-results.html)                                                                             |
| [mysqli::multi\_query()](mysqli.multi-query.html)                                                                               |
| [mysqli::next\_result()](mysqli.next-result.html)                                                                               |
| [mysqli::options()](mysqli.options.html)                                                                                        |
| [mysqli::ping()](mysqli.ping.html)                                                                                              |
| [mysqli::prepare()](mysqli.prepare.html)                                                                                        |
| [mysqli::query()](mysqli.query.html)                                                                                            |
| [mysqli::real\_connect()](mysqli.real-connect.html)                                                                             |
| [mysqli::real\_escape\_string()](mysqli.real-escape-string.html) [mysqli::escape\_string()](function.mysqli-escape-string.html) |
| [mysqli::real\_query()](mysqli.real-query.html)                                                                                 |
| [mysqli::refresh()](mysqli.refresh.html)                                                                                        |
| [mysqli::rollback()](mysqli.rollback.html)                                                                                      |
| [mysqli::select\_db()](mysqli.select-db.html)                                                                                   |
| [mysqli::set\_charset()](mysqli.set-charset.html)                                                                               |
| [mysqli::ssl\_set()](mysqli.ssl-set.html)                                                                                       |
| [mysqli::stat()](mysqli.stat.html)                                                                                              |
| [mysqli::stmt\_init()](mysqli.stmt-init.html)                                                                                   |
| [mysqli::store\_result()](mysqli.store-result.html)                                                                             |
| [mysqli::thread\_id()](mysqli.thread-id.html)                                                                                   |
| [mysqli::thread\_safe()](mysqli.thread-safe.html)                                                                               |
| [mysqli::use\_result()](mysqli.use-result.html)                                                                                 |

**Основна інформація про методи [mysqli\_stmt](class.mysqli-stmt.html)**

| MySQL\_STMT                                                          |
|----------------------------------------------------------------------|
| ООП-інтерфейс                                                        |
|                                                                      |
| *Властивості*                                                        |
| [$mysqli\_stmt::affected\_rows](mysqli-stmt.affected-rows.html)      |
| [$mysqli\_stmt::errno](mysqli-stmt.errno.html)                       |
| [$mysqli\_stmt::error](mysqli-stmt.error.html)                       |
| [$mysqli\_stmt::field\_count](mysqli-stmt.field-count.html)          |
| [$mysqli\_stmt::insert\_id](mysqli-stmt.insert-id.html)              |
| [$mysqli\_stmt::num\_rows](mysqli-stmt.num-rows.html)                |
| [$mysqli\_stmt::param\_count](mysqli-stmt.param-count.html)          |
| [$mysqli\_stmt::sqlstate](mysqli-stmt.sqlstate.html)                 |
| *Методи*                                                             |
| [mysqli\_stmt::attr\_get()](mysqli-stmt.attr-get.html)               |
| [mysqli\_stmt::attr\_set()](mysqli-stmt.attr-set.html)               |
| [mysqli\_stmt::bind\_param()](mysqli-stmt.bind-param.html)           |
| [mysqli\_stmt::bind\_result()](mysqli-stmt.bind-result.html)         |
| [mysqli\_stmt::close()](mysqli-stmt.close.html)                      |
| [mysqli\_stmt::data\_seek()](mysqli-stmt.data-seek.html)             |
| [mysqli\_stmt::execute()](mysqli-stmt.execute.html)                  |
| [mysqli\_stmt::fetch()](mysqli-stmt.fetch.html)                      |
| [mysqli\_stmt::free\_result()](mysqli-stmt.free-result.html)         |
| [mysqli\_stmt::get\_result()](mysqli-stmt.get-result.html)           |
| [mysqli\_stmt::get\_warnings()](mysqli-stmt.get-warnings.html)       |
| [mysqli\_stmt::more\_results()](mysqli-stmt.more-results.html)       |
| [mysqli\_stmt::next\_result()](mysqli-stmt.next-result.html)         |
| [mysqli\_stmt::num\_rows()](mysqli-stmt.num-rows.html)               |
| [mysqli\_stmt::prepare()](mysqli-stmt.prepare.html)                  |
| [mysqli\_stmt::reset()](mysqli-stmt.reset.html)                      |
| [mysqli\_stmt::result\_metadata()](mysqli-stmt.result-metadata.html) |
| [mysqli\_stmt::send\_long\_data()](mysqli-stmt.send-long-data.html)  |
| [mysqli\_stmt::store\_result()](mysqli-stmt.store-result.html)       |

**Основна інформація про методи [mysqli\_result](class.mysqli-result.html)**

| mysqli\_result                                                                                                                                             |
|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ООП-інтерфейс                                                                                                                                              |
|                                                                                                                                                            |
| *Властивості*                                                                                                                                              |
| [$mysqli\_result::current\_field](mysqli-result.current-field.html)                                                                                        |
| [$mysqli\_result::field\_count](mysqli-result.field-count.html)                                                                                            |
| [$mysqli\_result::lengths](mysqli-result.lengths.html)                                                                                                     |
| [$mysqli\_result::num\_rows](mysqli-result.num-rows.html)                                                                                                  |
| *Методи*                                                                                                                                                   |
| [mysqli\_result::data\_seek()](mysqli-result.data-seek.html)                                                                                               |
| [mysqli\_result::fetch\_all()](mysqli-result.fetch-all.html)                                                                                               |
| [mysqli\_result::fetch\_array()](mysqli-result.fetch-array.html)                                                                                           |
| [mysqli\_result::fetch\_assoc()](mysqli-result.fetch-assoc.html)                                                                                           |
| [mysqli\_result::fetch\_column()](mysqli-result.fetch-column.html)                                                                                         |
| [mysqli\_result::fetch\_field\_direct()](mysqli-result.fetch-field-direct.html)                                                                            |
| [mysqli\_result::fetch\_field()](mysqli-result.fetch-field.html)                                                                                           |
| [mysqli\_result::fetch\_fields()](mysqli-result.fetch-fields.html)                                                                                         |
| [mysqli\_result::fetch\_object()](mysqli-result.fetch-object.html)                                                                                         |
| [mysqli\_result::fetch\_row()](mysqli-result.fetch-row.html)                                                                                               |
| [mysqli\_result::field\_seek()](mysqli-result.field-seek.html)                                                                                             |
| [mysqli\_result::free()](mysqli-result.free.html) [mysqli\_result::close](mysqli-result.free.html) [mysqli\_result::free\_result](mysqli-result.free.html) |

**Основна інформація про методи [mysqli\_driver](class.mysqli-driver.html)**

| MySQL\_Driver                                                                         |
|---------------------------------------------------------------------------------------|
| ООП-інтерфейс                                                                         |
|                                                                                       |
| *Властивості*                                                                         |
| [$mysqli\_driver::mysqli\_report](mysqli-driver.report-mode.html)                     |
| *Методи*                                                                              |
| [mysqli\_driver::embedded\_server\_end()](mysqli-driver.embedded-server-end.html)     |
| [mysqli\_driver::embedded\_server\_start()](mysqli-driver.embedded-server-start.html) |

> **Зауваження**
> 
> Псевдоніми функцій призначені лише забезпечення зворотної сумісності. Не використовуйте їх у нових проектах.