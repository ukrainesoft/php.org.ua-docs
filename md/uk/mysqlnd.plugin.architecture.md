...
title: Архітектура плагінів MySQL Native Driver
---
...
На відміну від мов як C++ чи Java, в C немає фіксованих правил успадкування. Однак існують деякі домовленості, які необхідно дотримуватися, але це ми обговоримо пізніше.
...
Плагін `mysqlnd` працює перехоплюючи виклики модулів, які використовують `mysqlnd`, до `mysqlnd`. Це досягається заміною таблиці функцій `mysqlnd` на створену плагіном.
...
> **Зауваження**
> 
> Не використовуйте будь-яку логіку, пов'язану з фіксованим розміром при маніпуляції з таблицею функцій `mysqlnd`. Завжди додавайте нові методи наприкінці таблиці, оскільки сама таблиця може у майбутньому будь-якої миті змінитися.
...
У деяких випадках, наприклад, для `Connection::stmt_init()`, життєво важливо спочатку викликати батьківський метод, і тільки потім робити щось у новому методі.
...
Деякі класи містять покажчик таблицю функцій методів. Усі екземпляри подібних класів повинні ділити одну й ту саму таблицю функцій. Щоб уникнути хаосу, особливо у багатопоточному оточенні, керувати такими таблицями функцій варто лише під час MINIT.
...
Перевага таблиці функцій, що розділяється, у продуктивності, тому що немає потреби копіювати таблицю функцій окремо для кожного об'єкта.
...
| | Методи результуючого набору (MYSQLNDRESMETADATA) | Connection::resultmetainit() | Так, але викличте батька! | Result::readresultmetadata() | | Оператор (MYSQLNDSTMT) | Connection::stmtinit() | Так, але викличте батька! | Connection::stmtinit() | | Мережа (MYSQLNDNET) | mysqlndnetinit() | Ні | Connection::init() | | Протокол обміну (MYSQLNDPROTOCOL) | mysqlndprotocolinit() | Ні | Connection::init() |
...
Перелічені деструктори можуть не збігатися з актуальними методами `mysqlnd` для очищення самого об'єкту. Однак вони є найкращим місцем, куди ви можете вклинитися для очищення даних свого плагіна. Так само, як і з конструкторами, ви можете повністю перевизначити ці методи, але робити це не рекомендується. Якщо вам необхідно вставити в кожний із перерахованих методів очищення даних свого плагіна, необхідно забезпечити запуск батьківських методів `mysqlnd`
