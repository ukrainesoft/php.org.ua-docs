...
| **`PKCS7_NOINTERN`** | При перевірці повідомлення, сертифікати (якщо є), включені в повідомлення зазвичай перевіряються на наявність підпису. З цією опцією використовуються лише сертифікати, визначені у параметрі `extracerts` функції [openssl\_pkcs7\_verify()](function.openssl-pkcs7-verify.html). Інші сертифікати залишаються непідтвердженими центром сертифікації. |
