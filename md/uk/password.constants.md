...
-   `salt`(string) - сіль для використання під час створення хешу пароля. Зауважте, що завдання цієї опції запобігає автоматичній генерації солі.
    ...
    Якщо не задано, то для кожного пароля, що хешується, буде створена випадкова сіль за допомогою функції[password\_hash()](function.password-hash.md). Це є кращим способом, а з PHP 7.0.0 опція salt оголошена застарілою.
...
**`PASSWORD_ARGON2ID`** використовується для створення хеш паролів за допомогою алгоритму Argon2id. Підтримуються ті ж опції, що і для [**`PASSWORD_ARGON2I`**](password.constants.md#constant.password-argon2i)
