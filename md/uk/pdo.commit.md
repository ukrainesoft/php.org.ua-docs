...
`<?php/* Початок транзакції, відключення автоматичної фіксації */$dbh->beginTransaction();/* Вставка множини записів по принципу "все або нічого" */$sql =        VALUES (?, ?, ?)';$sth = $dbh->prepare($sql);foreach ($fruits as $fruit) {   $sth->execute(array(      | colour,        $fruit->calories,    ));;

**Приклад #2 Фіксація DDL-транзакції**

` <?php/* Початок транзакції, відключення автоматичної фіксації */$dbh->beginTransaction();/* Зміна схеми бази даних */$sth = $dbh->exec("DROP TABLE| */$dbh->commit();/* З'єднання з базою даних знов в режимі автоматичної фіксації */?> `
