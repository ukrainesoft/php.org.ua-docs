...
navigation:
  ...
  - pdo.connections.md: Подключения и управление подключениями »
  - index.md: PHP Manual
  - book.pdo.md: PDO
...
> **Зауваження**: Якщо в класі немає властивості з необхідним ім'ям, буде викликаний магічний метод [**set()**](language.oop5.overloading.md#language.oop5.overloading.members)
...
Детально про цей атрибут дивіться розділ [Ошибки и их обработка](pdo.error-handling.md)
...
Запитувати постійне з'єднання замість створення нового підключення. Докладніше про цей атрибут дивіться розділ [Подключения и управление подключениями](pdo.connections.md)
...
Припис не викидати винятків у разі помилок. Передбачається, що розробник скрипту явно перевіряє усі виняткові ситуації. Це стандартний режим. Докладніше про цей атрибут дивіться в розділі [Ошибки и их обработка](pdo.error-handling.md)

**`PDO::ERRMODE_WARNING`** (int)

Припис видавати повідомлення про помилку рівня PHP **`E_WARNING`**. Докладніше про цей атрибут дивіться в розділі [Ошибки и их обработка](pdo.error-handling.md)

**`PDO::ERRMODE_EXCEPTION`** (int)

Припис викидати виняток [PDOException](class.pdoexception.md) у разі виникнення помилки. Докладніше про цей атрибут дивіться в розділі [Ошибки и их обработка](pdo.error-handling.md)
...
Розпорядження створити об'єкт [PDOStatement](class.pdostatement.md) із послідовним курсором. Послідовні курсори вибираються за замовчуванням, оскільки це найбільш загальний та швидкий спосіб доступу до даних у БД із PHP.

**`PDO::CURSOR_SCROLL`** (int)

Розпорядження створити об'єкт [PDOStatement](class.pdostatement.md) з курсором, що прокручується. Передача констант `PDO::FETCH_ORI_*` дозволяє задати режим роботи курсора, те, як вибиратимуться рядки з результуючого набору запиту.

**`PDO::ERR_NONE`** (string)

Відповідає коду SQLSTATE '00000'. Це означає, що SQL-запит успішно виконаний без помилок або попереджень. Ця константа впроваджена для зручності під час перевірки, чи є помилки, функціями [PDO::errorCode()](pdo.errorcode.md) або [PDOStatement::errorCode()](pdostatement.errorcode.md). Також можна перевіряти, чи є помилка всередині методу, порівнюючи код повернення з цією константою.
...
Вказує, що функція створена [PDO::sqliteCreateFunction()](pdo.sqlitecreatefunction.md) детермінована, тобто вона завжди повертає однаковий результат для однакових вхідних даних в одному виразі SQL. (Доступно з PHP 7.1.4.)
