...
public **PDO::cubrid_schema**(int `$schema_type`, string `$table_name` =
?, string `$col_name` = ?): array
...
` <?php$pk_list = $dbh->cubrid_schema(PDO::CUBRID_SCH_PRIMARY_KEY, "game");print_r($pk_list);$fk_list = $dbh->cubrid_schema(PDO::CUBRID_SCH" $fk_list);?> `
