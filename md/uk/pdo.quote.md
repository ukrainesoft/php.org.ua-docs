...
Якщо ви використовуєте цю функцію для побудови SQL запитів, *наполегливо* рекомендується користуватися методом [PDO::prepare()](pdo.prepare.html) для підготовки запиту з псевдозмінними замість використання **PDO::quote()** для вставки даних, введених користувачем у SQL запит. Підготовлювані запити з параметрами не тільки компактнішими, зручнішими, стійкішими до SQL-ін'єкції, але й працюють швидше, ніж вручну побудовані запити, оскільки клієнт і сервер можуть кешувати такі запити у вже скомпілюваному вигляді.
...
Для коректної роботи **PDO::quote()** набір символів має бути заданий або сервері, або задаватися самим з'єднанням з базою даних (залежить від драйвера). Детальніше дивіться [документацию к драйверу базы данных](pdo.drivers.html)
