...
` <?php/* Починаємо транзакцію, відключення автоматичної фіксації */$dbh->beginTransaction();/* Змінюємо схему бази даних і даних в таблицях */$sth >  $sth = $dbh->exec("UPDATE dessert    SET name = 'hamburger'");/* Усвідомлюємо свою помилку і відкочуємо транзакцію *|$повертається| > `
