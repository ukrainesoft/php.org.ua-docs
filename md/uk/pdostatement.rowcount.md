...
` <?php/* Видалимо всі рядки з таблиці FRUIT */$del = $dbh->prepare('DELETE FROM fruit');$del->execute();/* Виведемо число віддалених віддалених рядків:
");$count = $del->rowCount();print("Видалено $count рядків.")
");?> `
...
` <?php$sql = "SELECT COUNT(*) FROM fruit WHERE calories > 100";$res = $conn->query($sql);$count = $res->fetchColumn();print "Спис" " . $count;?> `
