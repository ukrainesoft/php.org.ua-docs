...
Пароль використовується для шифрування поточного елемента. Якщо елемент не зашифрований, цей параметр не буде використаний і його можна не вказувати. Якщо цей параметр не вказано, а елемент зашифрований, то буде використаний пароль, переданий функції [raropen()](rararchive.open.html), Якщо її викликали. Якщо передано невірний пароль, явно чи неявно через [raropen()](rararchive.open.html), то перевірка CRC буде невдалою і буде повернуто **`false`**. Ви можете перевірити, чи є елемент зашифрованим за допомогою [RarEntry::isEncrypted()](rarentry.isencrypted.html)
