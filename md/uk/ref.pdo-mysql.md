...
Використовуйте **--with-pdo-mysql[=DIR]** для встановлення модуля PDO MySQL,
де необов'язковий параметр `[=DIR]` вказує директорію, де
встановлена MySQL. За замовчуванням використовується бібліотека [mysqlnd](book.mysqlnd.md). Більш детальний розбір на вибір
бібліотеки можна почитати у розділі "[Вибір бібліотеки MySQL](mysqlinfo.library.choosing.md)".

Додатковий параметр **--with-mysql-sock[=DIR]** вказує
...
Додатковий параметр **--with-zlib-dir[=DIR]** використовується як
префікс шляху до libz.

$ ./configure --with-pdo-mysql --with-mysql-sock=/var/mysql/mysql.sock

Підтримка SSL включається, використовуючи відповідну [константу PDO_MySQL](ref.pdo-mysql.md#pdo-mysql.constants), яка еквівалентна
...
до MySQL з SSL](https://dev.mysql.com/doc/en/using-encrypted-connections.md).
...
` <?phpif ($db->getAttribute(PDO::ATTR_DRIVER_NAME) == 'mysql') {   $stmt =$$db->prepare('select *fromfou': ;} else {    die("додаток працює тільки з mysql; Слід використовувати \$stmt->fetchAll() замість цього");}?> `
