...
-   Пакет`package`, включаючи`require()`и`module()`, оскільки він залежить від прямого доступу до файлової системі. Замість нього можна використовувати перезапис на чистому Lua, такий як той, що використовується в модулі MediaWiki Scribunto.
    
-   `load()`и`loadstring()`для статичного аналізу коду Lua.
...
-   Більшість пакету`os`оскільки він дозволяє керувати процесом і виконувати інші процеси.
    ...
    -   `os.clock()` `os.date()` `os.difftime()`и`os.time()`залишаються доступними.
...
-   `pcall()`и`xpcall()`неспроможна відловити певні помилки, особливо помилки часу очікування.
...
-   `math.random()`и`math.randomseed()`замінюються версіями, які не поділяють стан із функцією PHP`rand()`
    
-   Метаметоди`__pairs`и`__ipairs`Lua 5.2 підтримуються за допомогою`pairs()`и`ipairs()`
