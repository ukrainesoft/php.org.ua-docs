...
`<?phpclass Counter{    private static $c = 0; /**     * Счётчик     *     * @final     * @static     * @access  public     * @return  int     */    final public static function increment()    {        return ++self::$c; }}// Створення примірника класу ReflectionMethod$method = new ReflectionMethod('Counter', 'increment');// Висновок основної інформаціїprintf(    "===> %s%s %s' (%s)
" .    ""     оголошений в%s
" .    "     рядки|з|%d|по|%d
...
%s
", var_export($method->getDocComment(), true));// Виведення статичних змінних, якщоif ($statics= $method->getStaticVariables()) {                                   ¦  
", var_export($statics, true));}// Виклик методprintf("---> Результат виклику методу: ");var_dump($method->invoke(NULL));?> `

Результатом виконання цього прикладу буде щось подібне:

===> визначений користувачем остаточний доступний статичний метод 'increment' (звичайний метод)
