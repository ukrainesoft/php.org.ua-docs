---
navigation:
  - class.sodiumexception.html: « SodiumException
  - refs.database.abstract.html: Рівні абстракції »
  - index.html: PHP Manual
  - funcref.html: Справочник функций
title: Модулі для роботи з базами даних
---
# Модулі для роботи з базами даних

-   [Рівні абстракції](refs.database.abstract.html)
    -   [DBA](book.dba.html) - Database (dbm-style) Abstraction Layer
    -   [ODBC](book.uodbc.html) - ODBC (Unified)
    -   [PDO](book.pdo.html) - Об'єкти даних PHP
-   [Модулі для роботи з базами даних окремих виробників](refs.database.vendors.html)
    -   [CUBRID](book.cubrid.html)
    -   [dBase](book.dbase.html)
    -   [Firebird/InterBase](book.ibase.html)
    -   [IBM DB2](book.ibm-db2.html) - IBM DB2, Cloudscape та Apache Derby
    -   [MongoDB](set.mongodb.html) - Драйвер MongoDB
    -   [MySQL](set.mysqlinfo.html) — MySQL драйвери та плагіни
    -   [OCI8](book.oci8.html) - Oracle OCI8
    -   [PostgreSQL](book.pgsql.html)
    -   [SQLite3](book.sqlite3.html)
    -   [SQLSRV](book.sqlsrv.html) - Драйвер СУБД Microsoft SQL Server для PHP
