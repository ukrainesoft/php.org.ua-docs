...
Твердження щодо *наступного тексту* починаються з (?= для
позитивних тверджень і с (?! для заперечних тверджень. Наприклад,
`\w+(?=;)` збігається зі словом, за яким слідує символ ';', але при
...
Затвердження щодо *попереднього тексту* починаються з (?\<= для
...
довжину. Таким чином шаблон `(?<=bullock|donkey)` коректний, але
...
рядків однакової довжини. Таке твердження як `(?<=ab(c|de))` не
...
використовуючи альтернативи на верхньому рівні: `(?<=abc|abde)` Затвердження
...
наприклад: `(?<=\d{3})(?<!999)foo` збігається з підрядком "foo", якою
...
`(?<=(?<!foo)bar)baz` відповідає підрядку "baz", якій
передує "bar", перед якою, у свою чергу, немає "foo", а
`(?<=\d{3}...(?<!999))foo` - зовсім інший шаблон, відповідний
