...
` <?php$subject = 'test';/* (?# можна використовувати для додавання коментарів без включення PCRE_EXTENDED */$match = preg_match('/te(?# this s) var_dump($match);/* Пробіли і # розглядаються як частина шаблону, якщо не включений PCRE_EXTENDED. */$match = preg_match('/te   #~~~~st /* Коли PCRE_EXTENDED включений, всі пробіли і все, следует за неекранованим # в того ж рядку, ігнорується. */$match = preg_match('~ match);
