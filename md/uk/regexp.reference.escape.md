...
символ з властивістю xx, докладніше дивіться [свойства unicode](regexp.reference.unicode.md)

*P{xx}*

символ без властивості xx, докладніше дивіться [свойства unicode](regexp.reference.unicode.md)
...
Спеціальні послідовності `\A` `\Z` і `\z` відрізняються від загальновживаних метасимволів початку рядка '^' і кінця рядка '$' (описаних у розділі [якоря](regexp.reference.anchors.md)) тим, що вони завжди збігаються або на самому початку або наприкінці рядка. На них ніяк не впливають опції [PCREMULTILINE](reference.pcre.pattern.modifiers.md) і [PCREDOLLARENDONLY](reference.pcre.pattern.modifiers.md). Різниця між `\Z` і `\z` у тому, що `\Z` відповідає позиції перед останнім символом у разі, якщо останній символ - переклад рядка, крім кінця кінця рядка. В той час як `\z` відповідає виключно кінцю даних.

Твердження `\G` є дійсним лише в тому випадку, якщо поточна позиція, що перевіряється, знаходиться на початку збігу, вказаного параметром `offset` функції [pregmatch()](function.preg-match.md). Вона відрізняється від `\A` при ненульовому значенні параметра `offset`
