...
Послідовність `\K` може бути використана для скидання початку збігу. Наприклад, шаблон `foo\Kbar` збігається з "foobar", але повідомить про те, що збігся тільки з "bar". Використання `\K`не мешает установке подмасок. Например, если шаблон`(foo)\Kbar` збігається з рядком "foobar", першою підмаскою все одно буде "foo".
