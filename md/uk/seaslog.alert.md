...
public static **SeasLog::alert**(string `$message`, array `$content` =
?, string `$logger` = ?): bool
...
` <?phpvar_dump(SeasLog::alert('log message'));//с contentvar_dump(SeasLog::alert('log message from {NAME}',array('NAME' => 'neeke'))); //з часовим loggervar_dump(SeasLog::alert('log message from {NAME}',array('NAME' => 'neeke'),'tmp_logger'));var_dump(SeasLog::getBuffer());?> `
