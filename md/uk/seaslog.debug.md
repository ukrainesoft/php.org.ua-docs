...
public static **SeasLog::debug**(string `$message`, array `$content` =
?, string `$logger` = ?): bool
...
` <?phpvar_dump(SeasLog::debug('log message'));//с contentvar_dump(SeasLog::debug('log message from {NAME}',array('NAME' => 'neeke'))); //з часовим loggervar_dump(SeasLog::debug('log message from {NAME}',array('NAME' => 'neeke'),'tmp_logger'));var_dump(SeasLog::getBuffer());?> `
