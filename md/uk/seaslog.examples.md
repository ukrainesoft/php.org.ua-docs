...
` <?php $basePath1 =SeasLog::getBasePath(); SeasLog::setBasePath('/log/base_test'); $basePath2 = SeasLog::getBasePath(); var_dump($basePath1,$basePath2); ?> `
...
` <?php$lastLogger1 =SeasLog::getLastLogger();SeasLog::setLogger('testModule/app1');$lastLogger2 = SeasLog::getLastLogger();var_dump($lastLogger1,$las?
...
` <?phpSeasLog::log(SEASLOG_ERROR,'тестова помилка, зроблена ::log');SeasLog::debug('налагодження {userName}',array('{userName}' => 'neeke'));SeasLog: :info('запис інформаційного повідомлення в журнал');SeasLog::notice('запис попередження в журнал');SeasLog::warning('сайт {website} не працює, будь ласка, можна!  array('{website}' => 'github.com','{action}' => 'rboot'));SeasLog::error('запис помилки в журнал');SeasLog::critical('відбулося що- те критичне');SeasLog::alert('так, це повідомлення {messageName}',array('{messageName}' => 'alertMSG'));SeasLog::emergency('Тільки що повністю вигорів | }',array('{note}' => 'Це жарт'));?> `

Шаблон за замовчуванням: *seaslog.default_template = "%T\|%L\|%P\|%Q
...
*seaslog.appender = 2* або *seaslog.appender = 3*
...
` <?php$countResult1 =SeasLog::analyzerCount();$countResult2 =SeasLog::analyzerCount(SEASLOG_WARNING);$countResult3 =SeasLog::analyzerCount(SEASLOG_ERROR,date('$ countResult1,$countResult2,$countResult3);?> `
...
` <?php$detailErrorArray = SeasLog::analyzerDetail(SEASLOG_ERROR);var_dump($detailErrorArray);var_dump(SeasLog::analyzerDetail(SEASLOG_ERROR,date('Ymd',time())));?> `
