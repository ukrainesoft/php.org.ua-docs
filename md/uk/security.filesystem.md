...
` <?php// Видалення файла з домашньої директорії користувача$username = $_POST['user_submitted_name'];$userfile = $_POST['user_submitted_filename'];$homedir ==""/home/$$ /$userfile");echo "Файл був віддалений!";?> `
...
` <?php// Видалення любого файла, доступного з PHP-скрипта. // "../etc"$userfile = $_POST['user_submitted_filename']; //"passwd"$homedir =="/home/$username"; // "/home/../etc"unlink("$homedir/$userfile"); // "/home/../etc/passwd"echo "Файл був віддалений!";?> `
...
` <?php// Видалення любого файла, до которого має доступ користувач,// під яким запущений PHP.$username = $_SERVER['REMOTE_USER']; // використання авторизації$userfile = basename($_POST['user_submitted_filename']);$homedir = "/home/$username";$filepath = "$homedir/$userfile";if (file_exists($filepath) && $filepath)) {   $logstring = "$filepath віддалений
";} else {    $logstring = "Не удалося видалити $filepath
...
` <?php$username     = $_SERVER['REMOTE_USER']; // використання авторизації$userfile     = $_POST['user_submitted_filename'];$homedir     = "/home/$username";$filepath     == "$homedir/$userfile"|| '/^(?:[a-z0-9_-]|\.(?!\.))+$/iD', $userfile)) {   die("Неправильне ім'я користувача або файл");}//etc ...?> `
