...
Сесії можуть запускатися вручну за допомогою функції [sessionstart()](function.session-start.md). Якщо директива [session.autostart](session.configuration.html#ini.session.auto-start) встановлена ​​в `1`, сесія автоматично запуститься на початку запиту.
...
> **Зауваження**
> 
> Сесії, які використовують файли (за замовчуванням у PHP), блокують файл сесії відразу при відкритті сесії функцією [sessionstart()](function.session-start.html) або опосередковано при вказівці [session.autostart](session.configuration.html#ini.session.auto-start). Після блокування, жодний інший скрипт не може отримати доступ до цього ж файлу сесії, доки він не буде закритий або при завершенні скрипту або виклику функції [sessionwriteclose()](function.session-write-close.md)
> 
