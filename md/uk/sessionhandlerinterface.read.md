...
Читає дані сесії зі сховища сесій та повертає результат. Викликається відразу після старту сесії або коли викликана [sessionstart()](function.session-start.md). Зверніть увагу, що перед викликом цього методу буде викликано функцію [SessionHandlerInterface::open()](sessionhandlerinterface.open.md)
...
Дані, що повертаються цим методом, будуть розшифровані всередині PHP, використовуючи метод десеріалізації, зазначений у [session.serializehandler](session.configuration.md#ini.session.serialize-handler). Отримані дані будуть використані для заповнення суперглобального масиву [SESSION](reserved.variables.session.md)

Зверніть увагу, що схема серіалізації даних не така, як у функції [unserialize()](function.unserialize.md), і отримати доступ до даних можна за допомогою функції [sessiondecode()](function.session-decode.md)
...
-   INI-директива [session.serializehandler](session.configuration.md#ini.session.serialize-handler)
