...
Визначає стиль кодування, який використовуватиметься клієнтом, використовуючи константи **`SOAP_ENCODED`**или**`SOAP_LITERAL`**Константа**`SOAP_ENCODED`** вказує на кодування з використанням типів, визначених у специфікації SOAP. Константа **`SOAP_LITERAL`** вказує на кодування з використанням схеми певною службою.
...
Если указан параметр`wsdl`, а также параметр[soap.wsdl\_cache\_enabled](soap.configuration.md#ini.soap.wsdl-cache-enabled) Увімкнено, цей параметр визначає тип кешування. Одне із значень: **`WSDL_CACHE_NONE`** **`WSDL_CACHE_DISK`** **`WSDL_CACHE_MEMORY`**или**`WSDL_CACHE_BOTH`**
...
**Пример #2 Пример использования**SoapClient::\_\_construct()\*\*\*\*
