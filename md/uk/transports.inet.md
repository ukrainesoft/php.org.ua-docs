...
Протоколи `ssl://` і `tls://` (доступні, тільки якщо підтримка openssl включена до PHP) є розширеннями `tcp://`, що доповнюють його SSL-шифрування.

`ssl://` буде намагатися використовувати з'єднання SSL V2 або SSL V3, залежно від можливостей та налаштувань віддаленого хоста . `sslv2://` і `sslv3://` дозволяють явно вказати використання SSL V2 чи SSL V3.