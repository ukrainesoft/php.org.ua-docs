...
Файли, що відкриваються для читання з використанням обгортки ogg://, розглядаються як стислий аудіопотік, що кодується з використанням кодека `OGG/Vorbis`. Аналогічно, файли, відкриті для запису або додавання через обгортку ogg://, записуються як стиснуті звукові дані. Функція [streamgetmetadata()](function.stream-get-meta-data.md), коли використовується з файлами `OGG/Vorbis` відкритими для читання, повертатиме різноманітну інформацію про потік, включаючи тег виробника `vendor`, коментарі `comments`, кількість каналів `channels`, частоту дискретизації `rate`, і діапазон частоти кодування, що описується: `bitrate_lower` `bitrate_upper` `bitrate_nominal` і `bitrate_window`
