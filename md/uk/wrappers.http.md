...
Надає доступ лише для читання файлів/ресурсів через HTTP. За промовчанням використовується HTTP 1.0 GET. Для підтримки віртуальних хостів на основі імен разом із запитом надсилається заголовок `Host:`. Якщо ви налаштували рядок [useragent](filesystem.configuration.md#ini.user-agent), використовуючи ваш файл php.ini або контекст потоку, вона також буде включена в запит.
