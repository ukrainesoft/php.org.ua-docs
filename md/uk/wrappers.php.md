...
типом вмісту `enctype="multipart/form-data"`.
...
| Назва                                                                  | Опис                                                                                                                                                    |    |
|------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|----|
| resource=<потік для фільтрації>                                        | Цей параметр є необхідним. Він показує потоку, що його потрібно відфільтрувати.                                                                         |    |
...
| <список фільтрів для застосування до обох ланцюжків читання та запису> | Будь-який список фільтрів, які використовуються без префіксів read= або write=, буде застосований до обох потоків на читання та запис при необхідності. |    |
...
` <?php// Установка межі в 5 MB.$fiveMBs = 5 * 1024 * 1024;$fp = fopen("php://temp/maxmemory:$fiveMBs", 'r+');fput| hello
...
`<?php/* Це просто еквівалентно: readfile("http://www.example.com"); так як на самому справі фільтри не вказані */readfile("php://filter/resource=http://www.example.com");?> `

**Приклад #3 php://filter/read=\<список фільтрів для застосування до ланцюжка
читання\>**

Цей параметр приймає один або більше імен фільтрів, розділених
вертикальною рисою `|`.

` <?php/* Цей скрипт виведе вміст  www.example.com повністю в верхньому реєстрі */readfile("php://filter/read=string.toupper/resource=http://www.example.com"); /* Цей скрипт робить теж саме, вверхній, але  буде также кодувати алгоритмом ROT13 */readfile("php://filter/read=string.toupper|string.rot13/resource=http:// );?> `

**Приклад #4 php://filter/write=\<список фільтрів для застосування до
ланцюжку запису\>**
